...
# Liver
Animals.BC1_DU.BF <- Animals.BC1_DU[which(Animals.BC1_DU[,4] == "Backfat"),]
...
BC1_DU.PORK_07 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Genes/PK07", paste0(Animals.BC1_DU.BF$SAMPLE_NAME, ".genes.results"))
# Names of the columns
names(BC1_DU.PORK_07) <- paste0("X", Animals.BC1_DU.BF$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring whit a number.

### IMPORT
tx.BC1_DU.BF <- tximport(BC1_DU.PORK_07, type ="rsem", txIn = FALSE, txOut = FALSE)

# Header of the imported counts
head(tx.BC1_DU.BF$counts)
...
tx.BC1_DU.BF$length[tx.BC1_DU.BF$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_DU.BF <- DESeqDataSetFromTximport(tx.BC1_DU.BF,
                              colData = Animals.BC1_DU.BF,
                              design = ~ Group) # This design is temporary. 


# Header of the counts now converted into postive integers: 
head(counts(dds.BC1_DU.BF)) # This counts are unormalized. 

# Storing the positive-integer counts into a count matrix
CMX_dds.BC1_DU.BF <- counts(dds.BC1_DU.BF)

# Replace of the 'X'
Features <- rownames(CMX_dds.BC1_DU.BF)
CMX_dds.BC1_DU.BF <- cbind(Features, CMX_dds.BC1_DU.BF) 
CMX_dds.BC1_DU.BF <- CMX_dds.BC1_DU.BF[,c("Features", rownames(colData(dds.BC1_DU.BF)[order(colData(dds.BC1_DU.BF)[,8]),]))]
colnames(CMX_dds.BC1_DU.BF) <- str_replace(colnames(CMX_dds.BC1_DU.BF),"X","")
colnames(CMX_dds.BC1_DU.BF)[1] <- "Feature"


colnames(CMX_dds.BC1_DU.BF) <- c("Feature", rownames(colData(dds.BC1_DU.BF)[order(colData(dds.BC1_DU.BF)[,8]),]))
write.table(CMX_dds.BC1_DU.BF, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/COUNTS.BC1_DU.backfat.csv", sep =";", row.names = FALSE)
################################################################################
# Removal of unutilized objects
rm(tx.BC1_DU.BF)
```
...
```{r pre_filtering}
# Number of samples
N <- nrow(colData(dds.BC1_DU.BF))
# Features to keep
keep <- rowSums(counts(dds.BC1_DU.BF)) >= N

a <- dim(dds.BC1_DU.BF)[1]
# Application of the pre-filtering over the dds object. 
dds.BC1_DU.BF <- dds.BC1_DU.BF[keep,]

(dim(dds.BC1_DU.BF)[1] - a) / a
################################################################################
# Removal of unutilized objects
#rm(keep)
```
...
# creates 'sj' or 'sij' 
dds.BC1_DU.BF <- estimateSizeFactors(dds.BC1_DU.BF)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_DU.BF)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_DU.BF))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_DU.BF.norm.c <- counts(dds.BC1_DU.BF, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_DU.BF.norm.c)
```
...
dds.BC1_DU.BF.pseudo <- normTransform(dds.BC1_DU.BF)
```
...
```{r}
vsd <- vst(dds.BC1_DU.BF, blind=FALSE)
# rlog transformation
rlg.dds.BC1_DU.BF <- rlog(dds.BC1_DU.BF, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_DU.BF.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_DU.BF))
...
select <- order(rowMeans(dds.BC1_DU.BF.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_DU.BF)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_DU.BF.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_DU.BF)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_DU.BF.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_DU.BF.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_DU.BF)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_DU.BF.pseudo$Group
colnames(rlg_d.Mtx) <- dds.BC1_DU.BF$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_DU.BF, intgroup=c("Group"), returnData=TRUE)
...
colData(dds.BC1_DU.BF)

# Definition of the L (LOW) group as the reference group:
dds.BC1_DU.BF$Group <- relevel(dds.BC1_DU.BF$Group, 
                                           ref = "L")
```
...
dds.BC1_DU.BF$Backcross <- factor(dds.BC1_DU.BF$Backcross)

# Update of the design:
design(dds.BC1_DU.BF) <- formula(~ Group)
...
# Confirmation:
design(dds.BC1_DU.BF)
```


```{r DEG_Analysis}
# Update of the 'dds' object with the DEG analysis
dds.BC1_DU.BF <- DESeq(dds.BC1_DU.BF)
# Results with a == 0.1
res.BC1_DU.BF <- results(dds.BC1_DU.BF)
# Results with a == 0.05
res.BC1_DU.BF.a_0.05 <- results(dds.BC1_DU.BF, alpha=0.05)
```
...
summary(res.BC1_DU.BF)

# Number of significant LFCs...
sum(res.BC1_DU.BF$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_DU.BF[which(res.BC1_DU.BF$padj < 0.1),]
resV1[which(abs(resV1$log2FoldChange) > 1.5), ]
```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_DU.BF.a_0.05)

# Number of significant LFCs...
sum(res.BC1_DU.BF.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$padj < 0.05),]

resV2 <- res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$padj < 0.1),]
...
write.table(as.data.frame(na.omit(res.BC1_DU.BF)), 
          file="BC1_DU.BF.Results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_DU.BF.a_0.05 <- as.data.frame(na.omit(res.BC1_DU.BF.a_0.05))

DFres.BC1_DU.BF.a_0.05 <- cbind(rownames(DFres.BC1_DU.BF.a_0.05), DFres.BC1_DU.BF.a_0.05)
colnames(DFres.BC1_DU.BF.a_0.05) <- c("feature", colnames(DFres.BC1_DU.BF.a_0.05)[2:length(DFres.BC1_DU.BF.a_0.05)])

write.table(DFres.BC1_DU.BF.a_0.05, row.names = FALSE, 
          file="BC1_DU.BF.Results_a_0.05.csv", sep = ";")
```
...
qq.chisq(-2*log(res.BC1_DU.BF$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_DU.BF_above_fc.12 <- res.BC1_DU.BF[which(res.BC1_DU.BF$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_DU.BF_below_fc.12 <- res.BC1_DU.BF[which(res.BC1_DU.BF$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_DU.BF_abv.blw_fc.12 <- rbind(res.BC1_DU.BF_above_fc.12, res.BC1_DU.BF_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.BF$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_DU.BF_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.BF_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.BF_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_DU.BF_above_fc.15 <- res.BC1_DU.BF[which(res.BC1_DU.BF$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_DU.BF_below_fc.15 <- res.BC1_DU.BF[which(res.BC1_DU.BF$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_DU.BF_abv.blw_fc.15 <- rbind(res.BC1_DU.BF_above_fc.15, res.BC1_DU.BF_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_DU.BF$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_DU.BF_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.BF_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.BF_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_DU.BF_05above_fc.12 <- res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_DU.BF_05below_fc.12 <- res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_DU.BF_05abv.blw_fc.12 <- rbind(res.BC1_DU.BF_05above_fc.12, res.BC1_DU.BF_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.BF.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_DU.BF_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.BF_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.BF_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_DU.BF_05above_fc.15 <- res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_DU.BF_05below_fc.15 <- res.BC1_DU.BF.a_0.05[which(res.BC1_DU.BF.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_DU.BF_05abv.blw_fc.15 <- rbind(res.BC1_DU.BF_05above_fc.15, res.BC1_DU.BF_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.BF.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_DU.BF_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.BF_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.BF_05below_fc.15$padj < 0.05, na.rm = TRUE)`


# Plot of means of normalized counts vs -log~10~ p.values
```{r p.value_mean}
plot(res.BC1_DU.BF$baseMean+1, -log10(res.BC1_DU.BF$pvalue),
...
```{r}
use <- res.BC1_DU.BF$baseMean > metadata(res.BC1_DU.BF)$filterThreshold
h1 <- hist(res.BC1_DU.BF$pvalue[!use], breaks=0:50/50, plot=FALSE)
h2 <- hist(res.BC1_DU.BF$pvalue[use], breaks=0:50/50, plot=FALSE)
...
#BC1_DU.BF_pValues_dist.Indep_filt_COLOR
```
...
Features <- rownames(res.BC1_DU.BF_05abv.blw_fc.15[which(res.BC1_DU.BF_05abv.blw_fc.15$padj < 0.05),])
Features

# Specific sample (column) order: LOW before HIGH in blocks. 
samples <- rownames(colData(dds.BC1_DU.BF)[order(dds.BC1_DU.BF$Group, decreasing = TRUE),])

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_DU.BF)[rownames(res.BC1_DU.BF_05abv.blw_fc.15[which(res.BC1_DU.BF_05abv.blw_fc.15$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_DU.BF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
