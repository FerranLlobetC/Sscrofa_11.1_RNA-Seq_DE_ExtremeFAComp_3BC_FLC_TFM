...
Animals.BC1_DU.BF <- Animals.BC1_DU.BF[order(Animals.BC1_DU.BF[,1]),]
...
cnt.BC1_DU.BF <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/COUNTS.BC1_DU.backfat.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_DU.BF) <- cnt.BC1_DU.BF[,1]

cnt.BC1_DU.BF <- cnt.BC1_DU.BF[,2:length(cnt.BC1_DU.BF)]

cnt.BC1_DU.BF <- cnt.BC1_DU.BF[,order(colnames(cnt.BC1_DU.BF))]
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_DU.BF <- DESeqDataSetFromMatrix(countData = cnt.BC1_DU.BF,
...
saveRDS(pcaData, file = "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_PCA/BC1_DU.BF.rds")
...
write.table(as.data.frame(na.omit(res.BC1_DU.BF)), 
          file="BC1_DU.BF.SF.Results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_DU.BF.a_0.05, row.names = FALSE, 
          file="BC1_DU.BF.SF.Results_a_0.05.csv", sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_DU.BF.SF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_DU.BF_05abv.blw_fc.12[which(res.BC1_DU.BF_05abv.blw_fc.12$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_DU.BF)[rownames(res.BC1_DU.BF_05abv.blw_fc.12[which(res.BC1_DU.BF_05abv.blw_fc.12$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_DU.BF.SF_counts_Signif0.05_FC_1.2.csv", row.names = FALSE, sep = ";")
```