...
# Selection of 'liver'
Animals.BC1_DU.BF <- Animals.BC1_DU[which(Animals.BC1_DU[,3] == "Backfat"),]
```
...
```{r load_counts}
cnt.BC1_DU.BF <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/COUNTS.BC1_DU.backfat.csv", sep = ";", row.names = 1)

# Conversion into a matrix
cnt.BC1_DU.BF <- as.matrix(cnt.BC1_DU.BF)
```
...
compare <- relevel(as.factor(Animals.BC1_DU.BF$Group), ref = "L")

################ CHECKING COLS count matrix == ROWS metadata ###################
# The while loop serves to see the OK prompt when the orders has been modified. 
while (TRUE) {
  if (levels(as.factor(rownames(Animals.BC1_DU.BF) == 
                       colnames(cnt.BC1_DU.BF))) == TRUE) {
  ...
  cnt.BC1_DU.BF <- cnt.BC1_DU.BF[,rownames(Animals.BC1_DU.BF)]
}
}

dge.BC1_DU.BF <- DGEList(cnt.BC1_DU.BF, group = compare)
dge.BC1_DU.BF
```
...
```{r filtering}
a <- dim(dge.BC1_DU.BF$counts)[1]

# Filtering
keep <- filterByExpr(dge.BC1_DU.BF)
dge.BC1_DU.BF <- dge.BC1_DU.BF[keep, , keep.lib.sizes=FALSE]

# Percentage of remove features
round(((dim(dge.BC1_DU.BF$counts)[1] - a) / a), 4)

# Re-computation of the library sizes
dge.BC1_DU.BF$samples$lib.size <- colSums(dge.BC1_DU.BF$counts)
```
...
DF <- cbind(rownames(dge.BC1_DU.BF$samples), dge.BC1_DU.BF$samples)
colnames(DF) <- c("Sample", colnames(DF[2:ncol(DF)]))

# Saving the object 'DF' to be used for later creation of lib sizes plots for 
# the thesis
saveRDS(DF, file = "/home/fllobet/Documents/RNA-Seq_TFM/Extra_figures/eR_LIB/BC1_DU.BF.rds")

ggplot(DF[order(Animals.BC1_DU.BF$Group),], aes(x= `rownames(dge.BC1_DU.BF$samples)`, y = lib.size*1e-6, fill = group)) + geom_bar(stat = "identity", col = "#4d5d53") + labs(title = "Libray size accross samples", subtitle = st, x = "Sample", y = "Library size (millions)") + scale_x_discrete(breaks = colnames(dge.BC1_DU.BF$samples[order(Animals.BC1_DU.BF$Group),]), labels = str_replace(str_replace(colnames(dge.BC1_DU.BF$samples[order(Animals.BC1_DU.BF$Group),]), "X", ""),"_BF", "")) + 
...
dge.BC1_DU.BF$samples <- calcNormFactors(dge.BC1_DU.BF)
dge.BC1_DU.BF$samples

# Readdition of the group data beause it gets removed in the previous step
dge.BC1_DU.BF$samples$group <- compare 
```
...
```{r common_dispersion}
dge.BC1_DU.BF <- estimateCommonDisp(dge.BC1_DU.BF)

# Value of the common dispersion:
dge.BC1_DU.BF$common.dispersion
sqrt(dge.BC1_DU.BF$common.dispersion)
```
...
df_all_pseudoCnt <- cbind(Animals.BC1_DU.BF, t(pseudoCnt.BC1_DU.BF))
...
```{r eda_05_MAplot}
# Manual MA-Plot
df_ma <- data.frame("A" = (pseudoCnt.BC1_DU.BF[,1] + pseudoCnt.BC1_DU.BF[, 2])/2,
                    "M" = pseudoCnt.BC1_DU.BF[,1] - pseudoCnt.BC1_DU.BF[,2])
...
df_ovd <- data.frame(mean = apply(cnt.BC1_DU.BF[, Animals.BC1_DU.BF$Group == "L"], 1, mean),
                     var = apply(cnt.BC1_DU.BF[, Animals.BC1_DU.BF$Group == "L"], 1, var))
...
df_hm <- as.matrix(dist(t(pseudoCnt.BC1_DU.BF)))
...
# SD for all the rows
rowSD <- apply(pseudoCnt.BC1_DU.BF, 1, sd)
# Ordering by SD
pseudoCnt.BC1_DU.BF.top500 <- pseudoCnt.BC1_DU.BF[order(rowSD,
                                                        decreasing = TRUE),]
# Selection of the top 500 more variable features
pseudoCnt.BC1_DU.BF.top500 <- pseudoCnt.BC1_DU.BF.top500[1:500,]
################################################################################
# Computation of the PCA
resPCA <- prcomp(pseudoCnt.BC1_DU.BF)
summary(resPCA)
resPCA$sdev
...
resPCA_pc12 <- cbind(resPCA_pc12, Animals.BC1_DU.BF)
...
PCA.plot <- ggplot(resPCA_pc12, aes(x = PC1, y = PC2, color = Group)) + 
  ...
  geom_text_repel(aes(label = str_replace(
    str_replace(rownames(Animals.BC1_DU.BF), "_LD", ""),"X", ""))) +
...
mds <- plotMDS(pseudoCnt.BC1_DU.BF.top500, plot = FALSE)
...
mds.eigen <- mds.eigen[rownames(Animals.BC1_DU.BF),]

# Addition of group data
mds.eigen <- cbind(mds.eigen, Animals.BC1_DU.BF[,7])
...
ggplot(mds.eigen, aes(x = V1, y = V2, color = Group, 
  ...
  color = guide_legend(title = "Group", override.aes = aes(label = "•", 
                                                           size = 8))) +
  theme_bw()
# plotMDS(pseudoCnt.BC1_DU.BF.top500, col = c(rgb(189/225,183/225,107/225),
...
#                                             rgb(47/225,132/225,124/225),
#                                             rgb(189/225,183/225,107/225),
#                                             rgb(47/225,132/225,124/225)), labels = str_replace(str_replace(colnames(pseudoCnt.BC1_DU.BF.top500), "X", ""),"_BF", ""))
...
res_BC1_DU.BF <- exactTest(dge.BC1_DU.BF)
# Vislualitzation of the top 10 most significant features
topTags(res_BC1_DU.BF)

################################################################################
# FDR Correction: Benjamini-Honchberg.
res_BC1_DU.BF$table <- 
  cbind(res_BC1_DU.BF$table, 
        p.adjust(res_BC1_DU.BF$table$PValue, method = "BH"))

# Confirmation of the performed comparison
res_BC1_DU.BF$comparison

################################################################################
# Storing of the results into a CSV file
res_BC1_DU.BF$table <- cbind(rownames(res_BC1_DU.BF$table), res_BC1_DU.BF$table)
res_BC1_DU.BF$table <- cbind(rownames(res_BC1_DU.BF$table), res_BC1_DU.BF$table)
res_BC1_DU.BF$table[,1:2] <- str_split_fixed(rownames(res_BC1_DU.BF$table),"_", n = 2) 

colnames(res_BC1_DU.BF$table) <- c("ENSEMBL ID", "Gene name", colnames(res_BC1_DU.BF$table)[3:6])

# BUSCARV over the lists - ENSEMBL ID

IDs_in_coro <- merge(res_BC1_DU.BF$table, coro.BC1_LD.BF, by="ENSEMBL ID")$`ENSEMBL ID`

# Printing of the number of mathcing features
length(IDs_in_coro)

# Addition of this info in the output CSV of the significant genes
L <- dim(res_BC1_DU.BF$table)[2]
for (i in 1:dim(res_BC1_DU.BF$table)[1]) {
  if (res_BC1_DU.BF$table[i,1] %in% IDs_in_coro) {
    res_BC1_DU.BF$table[i,L+1] <- "in Corominas et al. 2013"
    }
}
################################################################################
...
GNs_in_coro <- merge(res_BC1_DU.BF$table, coro.BC1_LD.BF, by="Gene name")$`Gene name`
# Printing of the number of mathcing features
length(GNs_in_coro)

L <- dim(res_BC1_DU.BF$table)[2]
for (i in 1:dim(res_BC1_DU.BF$table)[1]) {
  if (res_BC1_DU.BF$table[i, 2] %in% GNs_in_coro) {
    res_BC1_DU.BF$table[i, L + 1] <- "in Corominas et al. 2013"
  }
}
...
colnames(res_BC1_DU.BF$table) <- c(colnames(res_BC1_DU.BF$table)[1:5], "PADJValue", "Paper ID", "Paper Gene")

# Merging the 2 columns of 'Paper ID' and 'Paper Name' to paint the plot
res_BC1_DU.BF$table  <- res_BC1_DU.BF$table %>% mutate(`Paper ID` = coalesce(`Paper ID`,`Paper Gene`)) %>%
         select(colnames(res_BC1_DU.BF$table)[1:7])



################################################################################
# Storing the results into a CSV file 
colnames(res_BC1_DU.BF$table) <- c(colnames(res_BC1_DU.BF$table)[1:5], "PADJValue", "Paper")
write.table(res_BC1_DU.BF$table, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/edgeR/BC1_DU.BF_eResults.csv", sep = ";", row.names = FALSE)
```

```{r signficant_features}
# Return of significant features with a given alpha threshold:
signif_BC1_DU.BF <- decideTestsDGE(res_BC1_DU.BF, adjust.method = "BH", p.value = 0.05)

# Selection of the significant features
sel_signif_BC1_DU.BF <- which(signif_BC1_DU.BF[,1] != 0)

# Datafame of the Results + P.adjusted values of only the significant features
signif_BC1_DU.BF <- cbind(res_BC1_DU.BF$table[sel_signif_BC1_DU.BF,],
                          signif_BC1_DU.BF[sel_signif_BC1_DU.BF, 1])

################################################################################
# Printing of:
# 'n' of signficant features (rows)
dim(signif_BC1_DU.BF)[1]

# 'n' of logFC > 0 

dim(signif_BC1_DU.BF[which(signif_BC1_DU.BF$logFC > 0 & signif_BC1_DU.BF$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_DU.BF[which(signif_BC1_DU.BF$logFC < 0 & signif_BC1_DU.BF$PADJValue < 0.05),])[1]

# Storing the signficiant features into a CSV file
write.table(signif_BC1_DU.BF, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/edgeR/BC1_DU.BF_eResults_Signif.05.csv", sep = ";", row.names = FALSE)
```
...
# Filtering 
signif_BC1_DU.BF_FC_1.2 <- signif_BC1_DU.BF[which(signif_BC1_DU.BF$logFC > fc12 | signif_BC1_DU.BF$logFC < -fc12),]
dim(signif_BC1_DU.BF_FC_1.2)[1]

# 'n' of logFC > 0 
dim(signif_BC1_DU.BF_FC_1.2[which(signif_BC1_DU.BF_FC_1.2$logFC > 0 & signif_BC1_DU.BF_FC_1.2$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_DU.BF_FC_1.2[which(signif_BC1_DU.BF_FC_1.2$logFC < 0 & signif_BC1_DU.BF_FC_1.2$PADJValue < 0.05),])[1]
```
# ABS Fold-Change > 1.5
...
# Filtering 
signif_BC1_DU.BF_FC_1.5 <- signif_BC1_DU.BF[which(signif_BC1_DU.BF$logFC > fc15 | signif_BC1_DU.BF$logFC < -fc15),]
dim(signif_BC1_DU.BF_FC_1.5)[1]

# 'n' of logFC > 0 
dim(signif_BC1_DU.BF_FC_1.5[which(signif_BC1_DU.BF_FC_1.5$logFC > 0 & signif_BC1_DU.BF_FC_1.5$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_DU.BF_FC_1.5[which(signif_BC1_DU.BF_FC_1.5$logFC < 0 & signif_BC1_DU.BF_FC_1.5$PADJValue < 0.05),])[1]
```
# PValues
...
```{r}
levels(as.factor(res_BC1_DU.BF$table$Paper))
ggplot(res_BC1_DU.BF$table[which(res_BC1_DU.BF$table$PADJValue < 1),], 
  ...
  labs(x = "Adjusted PValues distribution", y = "Frequency", title = "PValues distribution of BC1_DU ~ BACKFAT", subtitle = "Location of the signficiant features of Corominas et al. 2013 alognside the adjuste PValue distribution", caption = "The dashed line repsent 0.05 and 0.1 respectively") + theme_bw() + 
...
CPM.BC1_DU.BF <- CPM.BC1_DU.BF[,rownames(Animals.BC1_DU.BF[order(Animals.BC1_DU.BF$Group, decreasing = TRUE),])]

# Division of the 'Features' column into 2 new columns
CPM.BC1_DU.BF <- cbind(rownames(CPM.BC1_DU.BF),
                       rownames(CPM.BC1_DU.BF),
                       CPM.BC1_DU.BF)

CPM.BC1_DU.BF[,1:2] <- str_split_fixed(rownames(CPM.BC1_DU.BF),"_", n = 2) 

colnames(CPM.BC1_DU.BF) <- c("ENSEMBL ID", "Gene name", str_replace(colnames(CPM.BC1_DU.BF)[3:dim(CPM.BC1_DU.BF)[2]],"X",""))

write.table(CPM.BC1_DU.BF, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.BF/edgeR/CPM.BC1_DU.BF.csv", sep = ";", row.names = FALSE) 
