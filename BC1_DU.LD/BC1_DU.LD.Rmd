---
title: "BC1_DU.Longissimus_dorsi.R"
author: "FLC"
date: '2022-05-02'
output:
...
BC1_DU.PORK_07 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Genes/PK07", paste0(Animals.BC1_DU.LD$SAMPLE_NAME, ".genes.results"))
# Names of the columns
names(BC1_DU.PORK_07) <- paste0("X", Animals.BC1_DU.LD$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring whit a number.

### IMPORT
tx.BC1_DU.LD <- tximport(BC1_DU.PORK_07, type ="rsem", txIn = FALSE, txOut = FALSE)

# Header of the imported counts
head(tx.BC1_DU.LD$counts)
...
tx.BC1_DU.LD$length[tx.BC1_DU.LD$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_DU.LD <- DESeqDataSetFromTximport(tx.BC1_DU.LD,
...
CMX_dds.BC1_DU.LD <- counts(dds.BC1_DU.LD)
CMX_dds.BC1_DU.LD <- CMX_dds.BC1_DU.LD[,rownames(colData(dds.BC1_DU.LD)[order(colData(dds.BC1_DU.LD)[,8]),])]
# Replace of the 'X'
colnames(CMX_dds.BC1_DU.LD) <- str_replace(colnames(CMX_dds.BC1_DU.LD),"X","")
CMX_dds.BC1_DU.LD <- cbind(rownames(CMX_dds.BC1_DU.LD), CMX_dds.BC1_DU.LD) 
colnames(CMX_dds.BC1_DU.LD) <- c("Feature", colnames(CMX_dds.BC1_DU.LD)[2:dim(CMX_dds.BC1_DU.LD)[2]])
write.table(CMX_dds.BC1_DU.LD, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.LD/COUNTS.BC1_DU.LD.csv", sep =";", row.names = FALSE)
################################################################################
# Removal of unutilized objects
rm(tx.BC1_DU.LD)
```
...
write.table(as.data.frame(na.omit(res.BC1_DU.LD)), 
          file="BC1_DU.LD.Results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_DU.LD.a_0.05, row.names = FALSE, 
          file="BC1_DU.LD.Results_a_0.05.csv", sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_DU.LD_counts_Signif0.05.csv", row.names = FALSE, sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_DU.LD_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
