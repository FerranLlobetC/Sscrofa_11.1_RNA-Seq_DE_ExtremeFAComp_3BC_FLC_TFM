...
BC1_DU.PORK_07 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK07", paste0(Animals.BC1_DU.iF.LD$SAMPLE_NAME, ".isoforms.results"))

# Names of the columns
names(BC1_DU.PORK_07) <- paste0("X", Animals.BC1_DU.iF.LD$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring with a number.

### IMPORT
tx.BC1_DU.iF.LD <- tximport(BC1_DU.PORK_07, type ="rsem", txIn = TRUE, txOut = TRUE)

# Header of the imported counts
head(tx.BC1_DU.iF.LD$counts)
...
tx.BC1_DU.iF.LD$length[tx.BC1_DU.iF.LD$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_DU.iF.LD <- DESeqDataSetFromTximport(tx.BC1_DU.iF.LD,
...
head(counts(dds.BC1_DU.iF.LD)) # This counts are unormalized.

# Storing the positive-integer counts into a count matrix
CMX_dds.BC1_DU.iF.LD <- counts(dds.BC1_DU.iF.LD)
colnames(CMX_dds.BC1_DU.iF.LD) <- str_replace(colnames(CMX_dds.BC1_DU.iF.LD),"X","")
CMX_dds.BC1_DU.iF.LD <- cbind(rownames(CMX_dds.BC1_DU.iF.LD), CMX_dds.BC1_DU.iF.LD) 
colnames(CMX_dds.BC1_DU.iF.LD) <- c("Feature", colnames(CMX_dds.BC1_DU.iF.LD)[2:dim(CMX_dds.BC1_DU.iF.LD)[2]])
write.table(CMX_dds.BC1_DU.iF.LD, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.LD/ISOFORMS/IsoFORM_COUNTS.BC1_DU.ld.csv", sep =";", row.names = FALSE)
```
