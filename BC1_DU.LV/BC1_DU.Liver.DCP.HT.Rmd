...
# Liver 
Animals.BC1_DU.LV <- Animals.BC1_DU[which(Animals.BC1_DU[,4] == "Liver"),] 
Animals.BC1_DU.LV

################################################################################
# Removal of unutilized objects
rm(Animals, Animals.BC1_DU)

# Fixing the missidentified sample
rownames(Animals.BC1_DU.LV)[3] <- "X70505_LV"

# Reordering in alphabetical order
Animals.BC1_DU.LV <- Animals.BC1_DU.LV[order(rownames(Animals.BC1_DU.LV)),]
```

## `HTSeq` counts loading`

```{r tximport_RSEM_counts_loading}
# Simple lloading of the counts matrix 
cnt.BC1_DU.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_DU.LV/BC1_DU.DCP.LV.csv", sep=";", row.names = 1)
# It is important to state that the row names are the 1st column. 
rownames(cnt.BC1_DU.LV) <- str_c(rownames(cnt.BC1_DU.LV), cnt.BC1_DU.LV[,1], cnt.BC1_DU.LV[,2], sep = "_")
# Also the gene name and chromosome colmuns should be removed.
cnt.BC1_DU.LV <- cnt.BC1_DU.LV[,3:length(cnt.BC1_DU.LV)]

# Extra 'HTSeq' information rows (lsat 5 rows) should be removed too:
rws <- nrow(cnt.BC1_DU.LV)[1]-5
cnt.BC1_DU.LV <- cnt.BC1_DU.LV[1:rws,]
```

## Generation of the dds obje__no_feature	3765861
__ambiguous	408733
__too_low_aQual	0
__not_aligned	1030682
__alignment_not_unique	668137
ct


```{r dds_object}
### Standard 'dds' object:
dds.BC1_DU.LV <- DESeqDataSetFromMatrix(countData = cnt.BC1_DU.LV,
                              colData = Animals.BC1_DU.LV,
                              design = ~ Group) # This design is temporary. 

# Header of the counts: 
head(counts(dds.BC1_DU.LV)) # This counts are unormalized.
...
```{r pre_filtering}
# Number of samples
N <- nrow(Animals.BC1_DU.LV)
# Features to keep
keep <- rowSums(counts(dds.BC1_DU.LV)) >= N


# Application of the pre-filtering over the dds object. 
dds.BC1_DU.LV <- dds.BC1_DU.LV[keep,]

# Percentage of removed features
(31098 - dim(dds.BC1_DU.LV)[1] ) / 31908
...
# creates 'sj' or 'sij' 
dds.BC1_DU.LV <- estimateSizeFactors(dds.BC1_DU.LV)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_DU.LV)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_DU.LV))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_DU.LV.norm.c <- counts(dds.BC1_DU.LV, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_DU.LV.norm.c)
```
...
dds.BC1_DU.LV.pseudo <- normTransform(dds.BC1_DU.LV)
```
...
```{r}
vsd <- vst(dds.BC1_DU.LV, blind=FALSE)
# rlog transformation
rlg.dds.BC1_DU.LV <- rlog(dds.BC1_DU.LV, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_DU.LV.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_DU.LV))
...
select <- order(rowMeans(dds.BC1_DU.LV.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_DU.LV)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_DU.LV.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_DU.LV)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_DU.LV.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_DU.LV.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_DU.LV)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_DU.LV.pseudo$Group
colnames(rlg_d.Mtx) <- rlg.dds.BC1_DU.LV$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_DU.LV, intgroup=c("Group"), returnData=TRUE)
...
colData(dds.BC1_DU.LV)

# Definition of the L (LOW) group as the reference group:
dds.BC1_DU.LV$Group <- relevel(dds.BC1_DU.LV$Group, 
                                           ref = "L")
```
...
dds.BC1_DU.LV <- DESeq(dds.BC1_DU.LV)
# Results with a == 0.1
res.BC1_DU.LV <- results(dds.BC1_DU.LV)
# Results with a == 0.05
res.BC1_DU.LV.a_0.05 <- results(dds.BC1_DU.LV, alpha=0.05)
```
...
summary(res.BC1_DU.LV)

# Number of significant LFCs...
sum(res.BC1_DU.LV$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_DU.LV[which(res.BC1_DU.LV$padj < 0.1),]
...
# Summary of the up and down regulated genes in this study:
summary(res.BC1_DU.LV.a_0.05)

# Number of significant LFCs...
sum(res.BC1_DU.LV.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$padj < 0.05),]

resV2 <- res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$padj < 0.1),]
...
#write.table(as.data.frame(na.omit(res.BC1_DU.LV)), 
#          file="BC1_DU.LV.DEGanalysis_results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_DU.LV.a_0.05 <- as.data.frame(na.omit(res.BC1_DU.LV.a_0.05))

DFres.BC1_DU.LV.a_0.05 <- cbind(rownames(DFres.BC1_DU.LV.a_0.05), DFres.BC1_DU.LV.a_0.05)
colnames(DFres.BC1_DU.LV.a_0.05) <- c("feature", colnames(DFres.BC1_DU.LV.a_0.05)[2:length(DFres.BC1_DU.LV.a_0.05)])

write.table(DFres.BC1_DU.LV.a_0.05, row.names = FALSE, 
          file="BC1_DU.LV.HTSEQ_DEGanalysis_results_a_0.05.csv", sep = ";")
```
...
qq.chisq(-2*log(res.BC1_DU.LV$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_DU.LV_above_fc.12 <- res.BC1_DU.LV[which(res.BC1_DU.LV$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_DU.LV_below_fc.12 <- res.BC1_DU.LV[which(res.BC1_DU.LV$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_DU.LV_abv.blw_fc.12 <- rbind(res.BC1_DU.LV_above_fc.12, res.BC1_DU.LV_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.LV$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_DU.LV_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.LV_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.LV_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_DU.LV_above_fc.15 <- res.BC1_DU.LV[which(res.BC1_DU.LV$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_DU.LV_below_fc.15 <- res.BC1_DU.LV[which(res.BC1_DU.LV$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_DU.LV_abv.blw_fc.15 <- rbind(res.BC1_DU.LV_above_fc.15, res.BC1_DU.LV_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_DU.LV$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_DU.LV_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.LV_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.LV_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_DU.LV_05above_fc.12 <- res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_DU.LV_05below_fc.12 <- res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_DU.LV_05abv.blw_fc.12 <- rbind(res.BC1_DU.LV_05above_fc.12, res.BC1_DU.LV_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.LV.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_DU.LV_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.LV_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.LV_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_DU.LV_05above_fc.15 <- res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_DU.LV_05below_fc.15 <- res.BC1_DU.LV.a_0.05[which(res.BC1_DU.LV.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_DU.LV_05abv.blw_fc.15 <- rbind(res.BC1_DU.LV_05above_fc.15, res.BC1_DU.LV_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_DU.LV.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_DU.LV_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_DU.LV_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_DU.LV_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
```{r p.value_mean}
plot(res.BC1_DU.LV$baseMean+1, -log10(res.BC1_DU.LV$pvalue),
...
```{r}
use <- res.BC1_DU.LV$baseMean > metadata(res.BC1_DU.LV)$filterThreshold
h1 <- hist(res.BC1_DU.LV$pvalue[!use], breaks=0:50/50, plot=FALSE)
h2 <- hist(res.BC1_DU.LV$pvalue[use], breaks=0:50/50, plot=FALSE)
...
#BC1_DU.LV_pValues_dist.Indep_filt_COLOR
```
...
Features <- str_split_fixed(rownames(res.BC1_DU.LV_abv.blw_fc.15[which(res.BC1_DU.LV_05abv.blw_fc.15$padj < 0.05),]),"_", 3)[,1:2]
Features <- paste(Features[,1], Features[,2], sep = "_")

# Specific sample (column) order: LOW before HIGH in blocks
samples <- c("70225_LV", "70240_LV", "70353_LV", "70407_LV", "70490_LV",	"70498_LV",	"70258_LV",	"70330_LV",	"70395_LV",	"70476_LV",	"70484_LV",	"70515_LV")

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- cnt.BC1_DU.LV[rownames(res.BC1_DU.LV_abv.blw_fc.15[which(res.BC1_DU.LV_05abv.blw_fc.15$padj < 0.05),]), paste0("X",samples)]
...
# Wiritn of the csv
write.table(df, "BC1_DU.LV_HTSeq_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
