...
# To select the Corominas animals I added a column to the parent 'Animals.csv'
Animals.BC1_LD.BF <- Animals.BC1_LD.BF[which(Animals.BC1_LD.BF[,9] == "Corominas"),]
...
cnt.BC1_LD.BF <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/COUNTS.BC1_LD.backfat.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_LD.BF) <- cnt.BC1_LD.BF[,1]

cnt.BC1_LD.BF <- cnt.BC1_LD.BF[,2:length(cnt.BC1_LD.BF)]

# Selection of the 3v3 animals from Corominas et. al. 2013
cnt.BC1_LD.BF <- cnt.BC1_LD.BF[,c("X4777_BF", "X70084_BF", "X70094_BF", 
                                  "X70096_BF", "X70136_BF", "X70153_BF")]
```
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.BF <- DESeqDataSetFromMatrix(countData = cnt.BC1_LD.BF,
...
saveRDS(pcaData, file = "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_PCA/BC1_LD.BF.rds")
...
write.table(as.data.frame(na.omit(res.BC1_LD.BF)), 
          file="BC1_PI.BF.SF_3v3.Corominas_DEGanalysis_results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_LD.BF.a_0.05, row.names = FALSE, 
          file="BC1_PI.BF.SF_3v3.Corominas_DEGanalysis_results_a_0.05.csv", sep = ";")
```
...
df <- df[,c("Features",rownames(colData(dds.BC1_LD.BF)[order(colData(dds.BC1_LD.BF)[,8]),]))]
# Replace of the 'X'
colnames(df) <- str_replace(colnames(df), "X", "")

# Wiritn of the csv
write.table(df, "BC1_LD.BF_SF.3v3.Corominas_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
