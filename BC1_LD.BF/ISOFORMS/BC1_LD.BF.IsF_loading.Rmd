...
BC1_LD.PORK_02 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK02", paste0(Animals.BC1_LD.iF.BF$SAMPLE_NAME, ".isoforms.results"))

# Names of the columns
names(BC1_LD.PORK_02) <- paste0("X", Animals.BC1_LD.iF.BF$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring with a number.

### IMPORT
tx.BC1_LD.iF.BF <- tximport(BC1_LD.PORK_02, type ="rsem", txIn = TRUE, txOut = TRUE)

# Header of the imported counts
head(tx.BC1_LD.iF.BF$counts)
...
tx.BC1_LD.iF.BF$length[tx.BC1_LD.iF.BF$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.iF.BF <- DESeqDataSetFromTximport(tx.BC1_LD.iF.BF,
...
head(counts(dds.BC1_LD.iF.BF)) # This counts are unormalized.

# Storing the positive-integer counts into a count matrix
CMX_dds.BC1_LD.iF.BF <- counts(dds.BC1_LD.iF.BF)
colnames(CMX_dds.BC1_LD.iF.BF) <- str_replace(colnames(CMX_dds.BC1_LD.iF.BF),"X","")
CMX_dds.BC1_LD.iF.BF <- cbind(rownames(CMX_dds.BC1_LD.iF.BF), CMX_dds.BC1_LD.iF.BF) 
colnames(CMX_dds.BC1_LD.iF.BF) <- c("Feature", colnames(CMX_dds.BC1_LD.iF.BF)[2:dim(CMX_dds.BC1_LD.iF.BF)[2]])
write.table(CMX_dds.BC1_LD.iF.BF, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/IsoFORM_COUNTS.BC1_LD.backfat.csv", sep =";", row.names = FALSE)
```
