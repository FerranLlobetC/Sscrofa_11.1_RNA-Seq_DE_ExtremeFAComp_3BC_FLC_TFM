...
Code used in bash to obtain the `RSEM_counts_BC1_LD.BF.matrix` in the workstation:
```{comment}
conda activate RNA-Seq
./RNA-Seq_11.1_FLC/scripts/RSEM_Matrix.sh 02_NEWFilenames_AllCOLS.csv ";" "Backcross-BC1_LD,Tissue-Backfat" isoforms ./RNA-Seq_11.1_FLC/scripts/ 7 ./PORK_02/COUNTS/ RSEM_counts_BC1_LD.BF
```

# Lodaing *expected counts* from RSEM
```{r rsem_Counts_LOADING}
RSEM_exCnt.BC1_LD.iF.BF <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/RSEM_counts_BC1_LD.BF.matrix", sep = "\t", row.names = 1)
# Modification of the colnames:
colnames(RSEM_exCnt.BC1_LD.iF.BF) <- str_replace(str_replace(colnames(RSEM_exCnt.BC1_LD.iF.BF),".isoforms.results",""),"^[.-_]+\\.","X")
View(RSEM_exCnt.BC1_LD.iF.BF)
```
...
RSEM_exCnt.BC1_LD.iF.BF <- RSEM_exCnt.BC1_LD.iF.BF[,rownames(Animals.BC1_LD.iF.BF)]

################################################################################
# Removal of unutilized objects
rm(Animals)
```
The group vector will be `r Animals.BC1_LD.iF.BF$Group`

# Removal of the animals un-used in Corominas et al. 2013
Because the sample was contaminaed with muscle tissue
```{r animalsubset}
...
# Confirmation that both objects follow the same order
colnames(RSEM_exCnt.BC1_LD.iF.BF) == rownames(Animals.BC1_LD.iF.BF)
```


# Isoform and gene names
```{r isoform_names}
iso_names <- rownames(RSEM_exCnt.BC1_LD.iF.BF)
```

```{r gene_names}
# The first 'isofroms.results' file of the data will be used to retrieve the gene names
file <- paste0("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK02/",str_replace(colnames(RSEM_exCnt.BC1_LD.iF.BF)[1],"X",""),".isoforms.results")
gene_names <- read.delim(file)$gene_id
```

# Normalization: Library Size Factor
```{r library_size}
libSzF.BC1_LD.iF.BF <- MedianNorm(RSEM_exCnt.BC1_LD.iF.BF)
```
...
```{r Ng_vector}
Ng_BC1_LD.iF.BF <- GetNg(IsoformName = iso_names, GeneName = gene_names)
# The ACTUAL Ng vector that will be feed to the EBSeq model fitting
NgV_BC1.Pi.iF.BF <- Ng_BC1_LD.iF.BF$IsoformNgTrun
...
groups <- relevel(as.factor(Animals.BC1_LD.iF.BF$Group), ref = "L")
...
# Model fiting
EB.BC1_LD.iF.BF <- EBTest(Data=as.matrix(RSEM_exCnt.BC1_LD.iF.BF), 
                          NgVector=NgV_BC1.Pi.iF.BF, 
                          Conditions=groups,
                          sizeFactors=as.vector(libSzF.BC1_LD.iF.BF), 
                          maxround=15)

EB.BC1_LD.iF.BF$Alpha
EB.BC1_LD.iF.BF$Beta
EB.BC1_LD.iF.BF$P
```
...
robust.EBRes.BC1_LD.iF.BF <- GetDEResults(EB.BC1_LD.iF.BF, FDR = 0.05, Method = "robust")

# Aoplying the classic method
classic.EBRes.BC1_LD.iF.BF <- GetDEResults(EB.BC1_LD.iF.BF, FDR = 0.05, Method = "classic")

# Nº of DE Isoforoms 
# Robust method
length(robust.EBRes.BC1_LD.iF.BF$DEfound)
# Classic method
length(classic.EBRes.BC1_LD.iF.BF$DEfound)
```
# Calculation of Fold-Changes
```{r FC}
FC.BC1_LD.iF.BF <- PostFC(EB.BC1_LD.iF.BF)
FC.BC1_LD.iF.BF <- FC.BC1_LD.iF.BF$RealFC
# TO GET NUMBER OF FILTERED ISOFOROMS
length(FC.BC1_LD.iF.BF)
length(classic.EBRes.BC1_LD.iF.BF$PPMat)

# Generation of a results matrix of the classic method:
classicalFC.PP_matrix <- cbind(names(FC.BC1_LD.iF.BF), names(FC.BC1_LD.iF.BF), FC.BC1_LD.iF.BF, -log2(FC.BC1_LD.iF.BF), classic.EBRes.BC1_LD.iF.BF$PPMat[which(rownames(classic.EBRes.BC1_LD.iF.BF$PPMat) %in% names(FC.BC1_LD.iF.BF)),])
...
write.table(classicalFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/EBSeq/BC1_LD.iF.BF_EBcl.Results.csv", sep = ";", row.names = FALSE)
# Results CSV of only the significant features
write.table(classicalFC.PP_matrix[which(rownames(classicalFC.PP_matrix) %in% classic.EBRes.BC1_LD.iF.BF$DEfound),], "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/EBSeq/BC1_LD.iF.BF_EBcl_SIGNIF.Results.csv", sep = ";", row.names = FALSE)


# Generation of a results matrix of the robust method:
robustFC.PP_matrix <- cbind(names(FC.BC1_LD.iF.BF), names(FC.BC1_LD.iF.BF), FC.BC1_LD.iF.BF, -log2(FC.BC1_LD.iF.BF), robust.EBRes.BC1_LD.iF.BF$PPMat[which(rownames(robust.EBRes.BC1_LD.iF.BF$PPMat) %in% names(FC.BC1_LD.iF.BF)),])
...
# Results CSV of all the features
write.table(robustFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/EBSeq/BC1_LD.iF.BF_EBrb.Results.csv", sep = ";", row.names = FALSE)

# Results CSV of only the significant features
write.table(robustFC.PP_matrix[which(rownames(robustFC.PP_matrix) %in% robust.EBRes.BC1_LD.iF.BF$DEfound),], "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/EBSeq/BC1_LD.iF.BF_EBrb_SIGNIF.Results.csv", sep = ";", row.names = FALSE)

# Results CSV of only the significant features
```
# Calculation of percentage of filtered features
```{r filtered_features}
(dim(RSEM_exCnt.BC1_LD.iF.BF)[1] - dim(EB.BC1_LD.iF.BF$PPMat)[1]) / dim(RSEM_exCnt.BC1_LD.iF.BF)[1]
...
robustFC_signif <- robustFC.PP_matrix[which(rownames(robustFC.PP_matrix) %in% robust.EBRes.BC1_LD.iF.BF$DEfound),]
...
```{r comp_DESeq2}
BC1_LD.iF.BF_Dq2 <- read.csv("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/BC1_LD.iF.BF.SF.Results_a_0.05.csv", sep = ";")

# Selection of just the significant faetures
BC1_LD.iF.BF_Dq2 <- BC1_LD.iF.BF_Dq2[which(BC1_LD.iF.BF_Dq2[,8] < 0.05),]

# Row names of both objects become the ENSMEBL ID
rownames(BC1_LD.iF.BF_Dq2) <- BC1_LD.iF.BF_Dq2$ENSEMLB.ID
rownames(robustFC_signif) <- robustFC_signif[,1]
# 'BUSCARV' between the 2 lists
Common <- merge(robustFC_signif, BC1_LD.iF.BF_Dq2, by = 0)
...
nrow(Common)/nrow(BC1_LD.iF.BF_Dq2)
...
signifRSEMCnts <- as.data.frame(RSEM_exCnt.BC1_LD.iF.BF[,
                                          colnames(RSEM_exCnt.BC1_LD.iF.BF[
                                            order(Animals.BC1_LD.iF.BF$Group, 
                                                  decreasing = TRUE)])])
...
write.table(signifRSEMCnts, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.BF/ISOFORMS/EBSeq/BC1_LD.iF.BF_EBrb_COUNTS.csv", sep = ";", 
            row.names = F)
```
