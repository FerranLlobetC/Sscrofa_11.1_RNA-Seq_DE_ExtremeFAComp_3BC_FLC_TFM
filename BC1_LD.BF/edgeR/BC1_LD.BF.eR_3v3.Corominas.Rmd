...
# the thesis
saveRDS(DF, file = "/home/fllobet/Documents/RNA-Seq_TFM/Extra_figures/eR_LIB/BC1_LD.BF.rds")

ggplot(DF[order(Animals$Group),], aes(x= Sample, y = lib.size*1e-6, fill = group)) + geom_bar(stat = "identity", col = "#4d5d53") + labs(title = "Libray size accross samples", subtitle = st, x = "Sample", y = "Library size (millions)") + scale_x_discrete(breaks = rownames(dge.BC1_LD.BF$samples[order(Animals.BC1_LD.BF$Group),]), labels = str_replace(str_replace(rownames(dge.BC1_LD.BF$samples[order(Animals.BC1_LD.BF$Group),]), "X", ""),"_BF", "")) + 
...
BC1_LD.pv.Corominas <- ggplot(res_BC1_LD.BF$table[which(res_BC1_LD.BF$table$PADJValue < 1),], 
       aes(x = PADJValue, fill =  `Paper ID`)) + 
  geom_histogram(color = "#4d5d53") + 
  geom_vline(xintercept = 0.051, linetype='dashed', col = '#2f847c', size = 1.5) +
  geom_text(aes(x = 0.055, y = 115), label = "0.05", col = '#2f847c', size = 3.5, nudge_x = 0.05) +
  scale_fill_manual(name = "", values = c("#ace1af"), na.value = "#bdb76b", 
                    labels = c("in Corominas et al. 2013", 
                               "NOT in Corominas et al. 2013")) + 
  scale_y_continuous(expand = c(0,0), limits = c(0, 125), breaks = seq(0,120,20)) +
...
# Saving of the plot in a 'rds' object to be jonied with the ohter 2 for the 
# master's thesis. 
saveRDS(BC1_LD.pv.Corominas, 
        file = "~/Documents/RNA-Seq_TFM/Extra_figures/BC1_LV_vs_publ/BC1_LD.BF_vs_Corominas.rds")
layout <- "
AABB
#CC#
"

BC1_LD.pv.Corominas + BC1_LD.pv.Corominas + BC1_LD.pv.Corominas + plot_layout(design = layout)
```
