---
title: "BC1_LD.Longissimus_dorsi.R"
author: "FLC"
date: '2022-05-02'
output:
...
BC1_LD.PORK_02 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Genes/PK02", paste0(Animals.BC1_LD.LD$SAMPLE_NAME, ".genes.results"))
# Names of the columns
names(BC1_LD.PORK_02) <- paste0("X", Animals.BC1_LD.LD$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring whit a number.

### IMPORT
tx.BC1_LD.LD <- tximport(BC1_LD.PORK_02, type ="rsem", txIn = FALSE, txOut = FALSE)

# Header of the imported counts
head(tx.BC1_LD.LD$counts)
...
tx.BC1_LD.LD$length[tx.BC1_LD.LD$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.LD <- DESeqDataSetFromTximport(tx.BC1_LD.LD,
...
CMX_dds.BC1_LD.LD <- counts(dds.BC1_LD.LD)
CMX_dds.BC1_LD.LD <- CMX_dds.BC1_LD.LD[,rownames(colData(dds.BC1_LD.LD)[order(colData(dds.BC1_LD.LD)[,8]),])]
# Replace of the 'X'
colnames(CMX_dds.BC1_LD.LD) <- str_replace(colnames(CMX_dds.BC1_LD.LD),"X","")
CMX_dds.BC1_LD.LD <- cbind(rownames(CMX_dds.BC1_LD.LD), CMX_dds.BC1_LD.LD) 
colnames(CMX_dds.BC1_LD.LD) <- c("Feature", colnames(CMX_dds.BC1_LD.LD)[2:dim(CMX_dds.BC1_LD.LD)[2]])
write.table(CMX_dds.BC1_LD.LD, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LD/COUNTS.BC1_LD.LD.csv", sep =";", row.names = FALSE)
################################################################################
# Removal of unutilized objects
rm(tx.BC1_LD.LD)
```
...
# Results with a == 0.05
res.BC1_LD.LD.a_0.05 <- results(dds.BC1_LD.LD, alpha=0.05)
```
...
write.table(as.data.frame(na.omit(res.BC1_LD.LD)), 
          file="BC1_LD.LD.DEGanalysis_results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_LD.LD.a_0.05, row.names = FALSE, 
          file="BC1_LD.LD.DEGanalysis_results_a_0.05.csv", sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_LD.LD_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
