...
# Liver
Animals.BC1_LD.LD <- Animals.BC1_LD[which(Animals.BC1_LD[,4] == "Longissimusdorsi"),]

Animals.BC1_LD.LD <- Animals.BC1_LD.LD[order(Animals.BC1_LD.LD[,1]),]
...
cnt.BC1_LD.LD <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LD/COUNTS.BC1_LD.LD.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_LD.LD) <- cnt.BC1_LD.LD[,1]

cnt.BC1_LD.LD <- cnt.BC1_LD.LD[,2:length(cnt.BC1_LD.LD)]

cnt.BC1_LD.LD <- cnt.BC1_LD.LD[,order(colnames(cnt.BC1_LD.LD))]
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.LD <- DESeqDataSetFromMatrix(countData = cnt.BC1_LD.LD,
                              colData = Animals.BC1_LD.LD,
                              design = ~ Group) # This design is temporary.

# Header of the counts now converted into postive integers: 
head(counts(dds.BC1_LD.LD)) # This counts are unormalized. 
...
```{r pre_filtering}
# Number of samples
N <- nrow(Animals.BC1_LD.LD)
# Features to keep
keep <- rowSums(counts(dds.BC1_LD.LD)) >= N

a <- dim(dds.BC1_LD.LD)[1]
# Application of the pre-filtering over the dds object. 
dds.BC1_LD.LD <- dds.BC1_LD.LD[keep,]

(dim(dds.BC1_LD.LD)[1] - a) / a
################################################################################
# Removal of unutilized objects
#rm(keep)
```
...
# creates 'sj' or 'sij' 
dds.BC1_LD.LD <- estimateSizeFactors(dds.BC1_LD.LD)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_LD.LD)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_LD.LD))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_LD.LD.norm.c <- counts(dds.BC1_LD.LD, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_LD.LD.norm.c)
```
...
dds.BC1_LD.LD.pseudo <- normTransform(dds.BC1_LD.LD)
```
...
```{r}
vsd <- vst(dds.BC1_LD.LD, blind=FALSE)
# rlog transformation
rlg.dds.BC1_LD.LD <- rlog(dds.BC1_LD.LD, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_LD.LD.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_LD.LD))
...
select <- order(rowMeans(dds.BC1_LD.LD.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_LD.LD)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_LD.LD.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_LD.LD)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_LD.LD.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_LD.LD.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_LD.LD)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_LD.LD.pseudo$Group
colnames(rlg_d.Mtx) <- dds.BC1_LD.LD$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_LD.LD, intgroup=c("Group"), returnData=TRUE)

# Saving the pcaData object for the thesis figures
saveRDS(pcaData, file = "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_PCA/BC1_LD.LD.rds")
...
colData(dds.BC1_LD.LD)

# Definition of the L (LOW) group as the reference group:
dds.BC1_LD.LD$Group <- relevel(dds.BC1_LD.LD$Group, 
                                           ref = "L")
```
...
dds.BC1_LD.LD$Backcross <- factor(dds.BC1_LD.LD$Backcross)

# Update of the design:
design(dds.BC1_LD.LD) <- formula(~ Group)
...
# Confirmation:
design(dds.BC1_LD.LD)
```


```{r DEG_Analysis}
# Update of the 'dds' object with the DEG analysis
dds.BC1_LD.LD <- DESeq(dds.BC1_LD.LD)
# Results with a == 0.1
res.BC1_LD.LD <- results(dds.BC1_LD.LD)
# Results with a == 0.05
res.BC1_LD.LD.a_0.05 <- results(dds.BC1_LD.LD, alpha=0.05, independentFiltering = FALSE)
```
...
summary(res.BC1_LD.LD)

# Number of significant LFCs...
sum(res.BC1_LD.LD$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_LD.LD[which(res.BC1_LD.LD$padj < 0.1),]
resV1[which(abs(resV1$log2FoldChange) > 1.5), ]
```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_LD.LD.a_0.05)

# Number of significant LFCs...
sum(res.BC1_LD.LD.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$padj < 0.05),]

resV2 <- res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$padj < 0.1),]
...
write.table(as.data.frame(na.omit(res.BC1_LD.LD)), 
          file="BC1_LD.LD.SF.Results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_LD.LD.a_0.05 <- as.data.frame(na.omit(res.BC1_LD.LD.a_0.05))

DFres.BC1_LD.LD.a_0.05 <- cbind(rownames(DFres.BC1_LD.LD.a_0.05), DFres.BC1_LD.LD.a_0.05)
colnames(DFres.BC1_LD.LD.a_0.05) <- c("feature", colnames(DFres.BC1_LD.LD.a_0.05)[2:length(DFres.BC1_LD.LD.a_0.05)])

write.table(DFres.BC1_LD.LD.a_0.05, row.names = FALSE, 
          file="BC1_LD.LD.SF.Results_a_0.05.csv", sep = ";")
```
...
qq.chisq(-2*log(res.BC1_LD.LD$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_LD.LD_above_fc.12 <- res.BC1_LD.LD[which(res.BC1_LD.LD$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.LD_below_fc.12 <- res.BC1_LD.LD[which(res.BC1_LD.LD$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.LD_abv.blw_fc.12 <- rbind(res.BC1_LD.LD_above_fc.12, res.BC1_LD.LD_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.LD_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LD_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LD_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_LD.LD_above_fc.15 <- res.BC1_LD.LD[which(res.BC1_LD.LD$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.LD_below_fc.15 <- res.BC1_LD.LD[which(res.BC1_LD.LD$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.LD_abv.blw_fc.15 <- rbind(res.BC1_LD.LD_above_fc.15, res.BC1_LD.LD_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_LD.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.LD_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LD_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LD_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_LD.LD_05above_fc.12 <- res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.LD_05below_fc.12 <- res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.LD_05abv.blw_fc.12 <- rbind(res.BC1_LD.LD_05above_fc.12, res.BC1_LD.LD_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.LD_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LD_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LD_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_LD.LD_05above_fc.15 <- res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.LD_05below_fc.15 <- res.BC1_LD.LD.a_0.05[which(res.BC1_LD.LD.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.LD_05abv.blw_fc.15 <- rbind(res.BC1_LD.LD_05above_fc.15, res.BC1_LD.LD_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.LD_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LD_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LD_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
Features <- rownames(res.BC1_LD.LD_05abv.blw_fc.15[which(res.BC1_LD.LD_05abv.blw_fc.15$padj < 0.05),])
Features

# Specific sample (column) order: LOW before HIGH in blocks. 
samples <- rownames(colData(dds.BC1_LD.LD)[order(dds.BC1_LD.LD$Group, decreasing = TRUE),])

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_LD.LD)[rownames(res.BC1_LD.LD_05abv.blw_fc.15[which(res.BC1_LD.LD_05abv.blw_fc.15$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_LD.LD.SF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_LD.LD_05abv.blw_fc.12[which(res.BC1_LD.LD_05abv.blw_fc.12$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_LD.LD)[rownames(res.BC1_LD.LD_05abv.blw_fc.12[which(res.BC1_LD.LD_05abv.blw_fc.12$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_LD.LD.SF_counts_Signif0.05_FC_1.2.csv", row.names = FALSE, sep = ";")
```