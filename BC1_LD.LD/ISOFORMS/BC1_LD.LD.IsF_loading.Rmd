...
BC1_LD.PORK_02 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK02", paste0(Animals.BC1_LD.iF.LD$SAMPLE_NAME, ".isoforms.results"))

# Names of the columns
names(BC1_LD.PORK_02) <- paste0("X", Animals.BC1_LD.iF.LD$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring with a number.

### IMPORT
tx.BC1_LD.iF.LD <- tximport(BC1_LD.PORK_02, type ="rsem", txIn = TRUE, txOut = TRUE)

# Header of the imported counts
head(tx.BC1_LD.iF.LD$counts)
...
tx.BC1_LD.iF.LD$length[tx.BC1_LD.iF.LD$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.iF.LD <- DESeqDataSetFromTximport(tx.BC1_LD.iF.LD,
                              colData = Animals.BC1_LD.iF.LD,
                              design = ~ Group) # This design is temporary. 

# Header of the counts now converted into postive integers: 
head(counts(dds.BC1_LD.iF.LD)) # This counts are unormalized.

# Storing the positive-integer counts into a count matrix
CMX_dds.BC1_LD.iF.LD <- counts(dds.BC1_LD.iF.LD)
colnames(CMX_dds.BC1_LD.iF.LD) <- str_replace(colnames(CMX_dds.BC1_LD.iF.LD),"X","")
CMX_dds.BC1_LD.iF.LD <- cbind(rownames(CMX_dds.BC1_LD.iF.LD), CMX_dds.BC1_LD.iF.LD) 
colnames(CMX_dds.BC1_LD.iF.LD) <- c("Feature", colnames(CMX_dds.BC1_LD.iF.LD)[2:dim(CMX_dds.BC1_LD.iF.LD)[2]])
write.table(CMX_dds.BC1_LD.iF.LD, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LD/IsoFORM_COUNTS.BC1_LD.ld.csv", sep =";", row.names = FALSE)
```
...
```{r pre_filtering}
# Number of samples
N <- nrow(Animals.BC1_LD.iF.LD)
# Features to keep
keep <- rowSums(counts(dds.BC1_LD.iF.LD)) >= N

a <- dim(dds.BC1_LD.iF.LD)[1]
# Application of the pre-filtering over the dds object. 
dds.BC1_LD.iF.LD <- dds.BC1_LD.iF.LD[keep,]

(dim(dds.BC1_LD.iF.LD)[1] - a) / a
################################################################################
# Removal of unutilized objects
#rm(keep)
```
...
# creates 'sj' or 'sij' 
dds.BC1_LD.iF.LD <- estimateSizeFactors(dds.BC1_LD.iF.LD)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_LD.iF.LD)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_LD.iF.LD))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_LD.iF.LD.norm.c <- counts(dds.BC1_LD.iF.LD, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_LD.iF.LD.norm.c)
```
...
dds.BC1_LD.iF.LD.pseudo <- normTransform(dds.BC1_LD.iF.LD)
```
...
```{r}
vsd <- vst(dds.BC1_LD.iF.LD, blind=FALSE)
# rlog transformation
rlg.dds.BC1_LD.iF.LD <- rlog(dds.BC1_LD.iF.LD, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_LD.iF.LD.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_LD.iF.LD))
...
select <- order(rowMeans(dds.BC1_LD.iF.LD.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_LD.iF.LD)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_LD.iF.LD.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_LD.iF.LD)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_LD.iF.LD.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_LD.iF.LD.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_LD.iF.LD)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_LD.iF.LD.pseudo$Group
colnames(rlg_d.Mtx) <- dds.BC1_LD.iF.LD$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_LD.iF.LD, intgroup=c("Group"), returnData=TRUE)
...
colData(dds.BC1_LD.iF.LD)

# Definition of the L (LOW) group as the reference group:
dds.BC1_LD.iF.LD$Group <- relevel(dds.BC1_LD.iF.LD$Group, 
                                           ref = "L")
```
...
dds.BC1_LD.iF.LD$Backcross <- factor(dds.BC1_LD.iF.LD$Backcross)

# Update of the design:
design(dds.BC1_LD.iF.LD) <- formula(~ Group)
...
# Confirmation:
design(dds.BC1_LD.iF.LD)
```


```{r DEG_Analysis}
# Update of the 'dds' object with the DEG analysis
dds.BC1_LD.iF.LD <- DESeq(dds.BC1_LD.iF.LD)
# Results with a == 0.1
res.BC1_LD.iF.LD <- results(dds.BC1_LD.iF.LD)
# Results with a == 0.05
res.BC1_LD.iF.LD.a_0.05 <- results(dds.BC1_LD.iF.LD, alpha=0.05)
```
...
summary(res.BC1_LD.iF.LD)

# Number of significant LFCs...
sum(res.BC1_LD.iF.LD$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$padj < 0.1),]
resV1[which(abs(resV1$log2FoldChange) > 1.5), ]
```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_LD.iF.LD.a_0.05)

# Number of significant LFCs...
sum(res.BC1_LD.iF.LD.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$padj < 0.05),]

resV2 <- res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$padj < 0.1),]
...
write.table(as.data.frame(na.omit(res.BC1_LD.iF.LD)), 
          file="BC1_LD.iF.LD.SF.Results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_LD.iF.LD.a_0.05 <- as.data.frame(na.omit(res.BC1_LD.iF.LD.a_0.05))

DFres.BC1_LD.iF.LD.a_0.05 <- cbind(rownames(DFres.BC1_LD.iF.LD.a_0.05), DFres.BC1_LD.iF.LD.a_0.05)
colnames(DFres.BC1_LD.iF.LD.a_0.05) <- c("feature", colnames(DFres.BC1_LD.iF.LD.a_0.05)[2:length(DFres.BC1_LD.iF.LD.a_0.05)])

write.table(DFres.BC1_LD.iF.LD.a_0.05, row.names = FALSE, 
          file="BC1_LD.iF.LD.SF.Results_a_0.05.csv", sep = ";")
```
...
plotMA(res.BC1_LD.iF.LD.a_0.05)
```
...
qq.chisq(-2*log(res.BC1_LD.iF.LD$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_LD.iF.LD_above_fc.12 <- res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.iF.LD_below_fc.12 <- res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.iF.LD_abv.blw_fc.12 <- rbind(res.BC1_LD.iF.LD_above_fc.12, res.BC1_LD.iF.LD_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.iF.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.iF.LD_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.iF.LD_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.iF.LD_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_LD.iF.LD_above_fc.15 <- res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.iF.LD_below_fc.15 <- res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.iF.LD_abv.blw_fc.15 <- rbind(res.BC1_LD.iF.LD_above_fc.15, res.BC1_LD.iF.LD_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_LD.iF.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.iF.LD_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.iF.LD_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.iF.LD_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_LD.iF.LD_05above_fc.12 <- res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.iF.LD_05below_fc.12 <- res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.iF.LD_05abv.blw_fc.12 <- rbind(res.BC1_LD.iF.LD_05above_fc.12, res.BC1_LD.iF.LD_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.iF.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.iF.LD_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.iF.LD_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.iF.LD_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_LD.iF.LD_05above_fc.15 <- res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.iF.LD_05below_fc.15 <- res.BC1_LD.iF.LD.a_0.05[which(res.BC1_LD.iF.LD.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.iF.LD_05abv.blw_fc.15 <- rbind(res.BC1_LD.iF.LD_05above_fc.15, res.BC1_LD.iF.LD_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.iF.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.iF.LD_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.iF.LD_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.iF.LD_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
Features <- rownames(res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$padj < 0.05),])
Features

# Specific sample (column) order: LOW before HIGH in blocks. 
samples <- rownames(colData(dds.BC1_LD.iF.LD)[order(dds.BC1_LD.iF.LD$Group, decreasing = TRUE),])

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_LD.iF.LD)[rownames(res.BC1_LD.iF.LD[which(res.BC1_LD.iF.LD$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_LD.iF.LD.SF_counts_Signif0.05.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_LD.iF.LD_05abv.blw_fc.15[which(res.BC1_LD.iF.LD_05abv.blw_fc.15$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_LD.iF.LD)[rownames(res.BC1_LD.iF.LD_05abv.blw_fc.15[which(res.BC1_LD.iF.LD_05abv.blw_fc.15$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_LD.iF.LD.SF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_LD.iF.LD_05abv.blw_fc.12[which(res.BC1_LD.iF.LD_05abv.blw_fc.12$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_LD.iF.LD)[rownames(res.BC1_LD.iF.LD_05abv.blw_fc.12[which(res.BC1_LD.iF.LD_05abv.blw_fc.12$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_LD.iF.LD.SF_counts_Signif0.05_FC_1.2.csv", row.names = FALSE, sep = ";")
```
