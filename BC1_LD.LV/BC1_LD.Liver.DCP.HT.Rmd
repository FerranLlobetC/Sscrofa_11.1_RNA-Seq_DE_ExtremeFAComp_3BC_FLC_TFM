...
# DCP only maintained the female samples (removed directly the males)
Animals.BC1_LD.LV <- Animals.BC1_LD.LV[which(Animals.BC1_LD.LV[,5] == "F"),]
Animals.BC1_LD.LV
...
Animals.BC1_LD.LV <- Animals.BC1_LD.LV[order(rownames(Animals.BC1_LD.LV)),]
```
...
cnt.BC1_LD.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LV/DCP/counts_BC1_LD.LV.DCP.csv", sep=";", row.names = 1)
# It is important to state that the row names are the 1st column. 
rownames(cnt.BC1_LD.LV) <- str_c(rownames(cnt.BC1_LD.LV), cnt.BC1_LD.LV[,1], cnt.BC1_LD.LV[,2], sep = "_")
# Also the gene name and chromosome colmuns should be removed.
cnt.BC1_LD.LV <- cnt.BC1_LD.LV[,3:length(cnt.BC1_LD.LV)]

# Extra 'HTSeq' information rows (lsat 5 rows) should be removed too:
rws <- nrow(cnt.BC1_LD.LV)[1]-5
cnt.BC1_LD.LV <- cnt.BC1_LD.LV[1:rws,]
```
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.LV <- DESeqDataSetFromMatrix(countData = cnt.BC1_LD.LV,
...
(31098 - dim(dds.BC1_LD.LV)[1] ) / 31908
...
rownames(pseudo_d.Mtx) <- dds.BC1_LD.LV.pseudo$Group
...
rownames(rlg_d.Mtx) <- dds.BC1_LD.LV.pseudo$Group
...
pcaData <- plotPCA(rlg.dds.BC1_LD.LV, intgroup=c("Group"), returnData=TRUE)
...
colData(dds.BC1_LD.LV)

# Definition of the L (LOW) group as the reference group:
dds.BC1_LD.LV$Group <- relevel(dds.BC1_LD.LV$Group, 
                                           ref = "L")
```
...
dds.BC1_LD.LV <- DESeq(dds.BC1_LD.LV)
# Results with a == 0.1
res.BC1_LD.LV <- results(dds.BC1_LD.LV)
# Results with a == 0.05
res.BC1_LD.LV.a_0.05 <- results(dds.BC1_LD.LV, alpha=0.05)
```
...
summary(res.BC1_LD.LV)

# Number of significant LFCs...
sum(res.BC1_LD.LV$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_LD.LV[which(res.BC1_LD.LV$padj < 0.1),]
resV1[which(abs(resV1$log2FoldChange) > 1.5), ]
```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_LD.LV.a_0.05)

# Number of significant LFCs...
sum(res.BC1_LD.LV.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$padj < 0.05),]

resV2 <- res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$padj < 0.1),]
...
#write.table(as.data.frame(na.omit(res.BC1_LD.LV)), 
#          file="BC1_LD.LV.DEGanalysis_results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_LD.LV.a_0.05 <- as.data.frame(na.omit(res.BC1_LD.LV.a_0.05))

DFres.BC1_LD.LV.a_0.05 <- cbind(rownames(DFres.BC1_LD.LV.a_0.05), DFres.BC1_LD.LV.a_0.05)
colnames(DFres.BC1_LD.LV.a_0.05) <- c("feature", colnames(DFres.BC1_LD.LV.a_0.05)[2:length(DFres.BC1_LD.LV.a_0.05)])

write.table(DFres.BC1_LD.LV.a_0.05, row.names = FALSE, 
          file="BC1_LD.LV.HTSEQ_DEGanalysis_results_a_0.05.csv", sep = ";")
```
...
```{r}
# MA plot with the p.value cutoff at .05
plotMA(res.BC1_LD.LV.a_0.05)
```
...
qq.chisq(-2*log(res.BC1_LD.LV$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_LD.LV_above_fc.12 <- res.BC1_LD.LV[which(res.BC1_LD.LV$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.LV_below_fc.12 <- res.BC1_LD.LV[which(res.BC1_LD.LV$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.LV_abv.blw_fc.12 <- rbind(res.BC1_LD.LV_above_fc.12, res.BC1_LD.LV_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LV$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.LV_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LV_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LV_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_LD.LV_above_fc.15 <- res.BC1_LD.LV[which(res.BC1_LD.LV$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.LV_below_fc.15 <- res.BC1_LD.LV[which(res.BC1_LD.LV$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.LV_abv.blw_fc.15 <- rbind(res.BC1_LD.LV_above_fc.15, res.BC1_LD.LV_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_LD.LV$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_LD.LV_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LV_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LV_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_LD.LV_05above_fc.12 <- res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_LD.LV_05below_fc.12 <- res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_LD.LV_05abv.blw_fc.12 <- rbind(res.BC1_LD.LV_05above_fc.12, res.BC1_LD.LV_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LV.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.LV_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LV_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LV_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_LD.LV_05above_fc.15 <- res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_LD.LV_05below_fc.15 <- res.BC1_LD.LV.a_0.05[which(res.BC1_LD.LV.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_LD.LV_05abv.blw_fc.15 <- rbind(res.BC1_LD.LV_05above_fc.15, res.BC1_LD.LV_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_LD.LV.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_LD.LV_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_LD.LV_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_LD.LV_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
```{r p.value_mean}
plot(res.BC1_LD.LV$baseMean+1, -log10(res.BC1_LD.LV$pvalue),
...
```{r}
use <- res.BC1_LD.LV$baseMean > metadata(res.BC1_LD.LV)$filterThreshold
h1 <- hist(res.BC1_LD.LV$pvalue[!use], breaks=0:50/50, plot=FALSE)
h2 <- hist(res.BC1_LD.LV$pvalue[use], breaks=0:50/50, plot=FALSE)
...
#BC1_LD.LV_pValues_dist.Indep_filt_COLOR
```
...
Features <- str_split_fixed(rownames(res.BC1_LD.LV_abv.blw_fc.15[which(res.BC1_LD.LV_05abv.blw_fc.15$padj < 0.05),]),"_", 3)[,1:2]
...
df <- cnt.BC1_LD.LV[rownames(res.BC1_LD.LV_abv.blw_fc.15[which(res.BC1_LD.LV_05abv.blw_fc.15$padj < 0.05),]), paste0("X",samples)]
...
# Wiritn of the csv
write.table(df, "BC1_LD.LV_HTSeq_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
