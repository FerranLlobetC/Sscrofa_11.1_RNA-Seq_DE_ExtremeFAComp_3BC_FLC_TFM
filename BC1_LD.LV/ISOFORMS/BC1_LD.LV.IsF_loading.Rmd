...
BC1_LD.PORK_02 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK02", paste0(Animals.BC1_LD.iF.LV$SAMPLE_NAME, ".isoforms.results"))

# Names of the columns
names(BC1_LD.PORK_02) <- paste0("X", Animals.BC1_LD.iF.LV$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring with a number.

### IMPORT
tx.BC1_LD.iF.LV <- tximport(BC1_LD.PORK_02, type ="rsem", txIn = TRUE, txOut = TRUE)

# Header of the imported counts
head(tx.BC1_LD.iF.LV$counts)
...
tx.BC1_LD.iF.LV$length[tx.BC1_LD.iF.LV$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_LD.iF.LV <- DESeqDataSetFromTximport(tx.BC1_LD.iF.LV,
...
head(counts(dds.BC1_LD.iF.LV)) # This counts are unormalized.

# Storing the positive-integer counts into a count matrix
CMX_dds.BC1_LD.iF.LV <- counts(dds.BC1_LD.iF.LV)
colnames(CMX_dds.BC1_LD.iF.LV) <- str_replace(colnames(CMX_dds.BC1_LD.iF.LV),"X","")
CMX_dds.BC1_LD.iF.LV <- cbind(rownames(CMX_dds.BC1_LD.iF.LV), CMX_dds.BC1_LD.iF.LV) 
colnames(CMX_dds.BC1_LD.iF.LV) <- c("Feature", colnames(CMX_dds.BC1_LD.iF.LV)[2:dim(CMX_dds.BC1_LD.iF.LV)[2]])
write.table(CMX_dds.BC1_LD.iF.LV, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LV/ISOFORMS/IsoFORM_COUNTS.BC1_LD.liver.csv", sep =";", row.names = FALSE)
```
