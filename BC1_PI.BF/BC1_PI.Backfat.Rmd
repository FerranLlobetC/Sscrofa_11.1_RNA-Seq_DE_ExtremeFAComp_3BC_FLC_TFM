...
BC1_PI.PORK_06 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Genes/PK06", paste0(Animals.BC1_PI.BF$SAMPLE_NAME, ".genes.results"))
# Names of the columns
names(BC1_PI.PORK_06) <- paste0("X", Animals.BC1_PI.BF$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring whit a number.

### IMPORT
tx.BC1_PI.BF <- tximport(BC1_PI.PORK_06, type ="rsem", txIn = FALSE, txOut = FALSE)

# Header of the imported counts
head(tx.BC1_PI.BF$counts)
...
tx.BC1_PI.BF$length[tx.BC1_PI.BF$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_PI.BF <- DESeqDataSetFromTximport(tx.BC1_PI.BF,
...
CMX_dds.BC1_PI.BF <- counts(dds.BC1_PI.BF)

# Replace of the 'X'
Features <- rownames(CMX_dds.BC1_PI.BF)
CMX_dds.BC1_PI.BF <- cbind(Features, CMX_dds.BC1_PI.BF) 
CMX_dds.BC1_PI.BF <- CMX_dds.BC1_PI.BF[,c("Features", rownames(colData(dds.BC1_PI.BF)[order(colData(dds.BC1_PI.BF)[,8]),]))]
colnames(CMX_dds.BC1_PI.BF) <- str_replace(colnames(CMX_dds.BC1_PI.BF),"X","")
colnames(CMX_dds.BC1_PI.BF)[1] <- "Feature"


colnames(CMX_dds.BC1_PI.BF) <- c("Feature", rownames(colData(dds.BC1_PI.BF)[order(colData(dds.BC1_PI.BF)[,8]),]))
write.table(CMX_dds.BC1_PI.BF, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.BF/COUNTS.BC1_PI.backfat.csv", sep =";", row.names = FALSE)
################################################################################
# Removal of unutilized objects
rm(tx.BC1_PI.BF)
```
...
write.table(as.data.frame(na.omit(res.BC1_PI.BF)), 
          file="BC1_PI.BF.Results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_PI.BF.a_0.05, row.names = FALSE, 
          file="BC1_PI.BF.Results_a_0.05.csv", sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_PI.BF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
