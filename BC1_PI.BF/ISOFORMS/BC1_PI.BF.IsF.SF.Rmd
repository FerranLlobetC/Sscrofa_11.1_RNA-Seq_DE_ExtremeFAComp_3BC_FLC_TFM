...
# Liver
Animals.BC1_PI.iF.BF <- Animals.BC1_PI[which(Animals.BC1_PI[,4] == "Backfat"),]

Animals.BC1_PI.iF.BF <- Animals.BC1_PI.iF.BF[order(Animals.BC1_PI.iF.BF[,1]),]
...
The uncertainty-containing decimal counts were converted into integer counts usinng `tximport` in the script `BC1_PI.BF.IsF_loading.Rmd`

```{r couts_LOADING}
cnt.BC1_PI.iF.BF <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.BF/ISOFORMS/IsoFORM_COUNTS.BC1_PI.backfat.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_PI.iF.BF) <- cnt.BC1_PI.iF.BF[,1]

cnt.BC1_PI.iF.BF <- cnt.BC1_PI.iF.BF[,2:length(cnt.BC1_PI.iF.BF)]

cnt.BC1_PI.iF.BF <- cnt.BC1_PI.iF.BF[,order(colnames(cnt.BC1_PI.iF.BF))]
...
Animals.BC1_PI.iF.BF <- Animals.BC1_PI.iF.BF[which(Animals.BC1_PI.iF.BF[,5] == "F"),]

# Seletion of the 6 animals from the counts matrix
cnt.BC1_PI.iF.BF <- cnt.BC1_PI.iF.BF[, rownames(Animals.BC1_PI.iF.BF)]
```
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_PI.iF.BF <- DESeqDataSetFromMatrix(cnt.BC1_PI.iF.BF,
                              colData = Animals.BC1_PI.iF.BF,
...
```{r pre_filtering}
# Number of samples
N <- nrow(Animals.BC1_PI.iF.BF)
# Features to keep
keep <- rowSums(counts(dds.BC1_PI.iF.BF)) >= N

a <- dim(dds.BC1_PI.iF.BF)[1]
# Application of the pre-filtering over the dds object. 
dds.BC1_PI.iF.BF <- dds.BC1_PI.iF.BF[keep,]

(dim(dds.BC1_PI.iF.BF)[1] - a) / a
################################################################################
# Removal of unutilized objects
#rm(keep)
```
...
# creates 'sj' or 'sij' 
dds.BC1_PI.iF.BF <- estimateSizeFactors(dds.BC1_PI.iF.BF)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_PI.iF.BF)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_PI.iF.BF))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_PI.iF.BF.norm.c <- counts(dds.BC1_PI.iF.BF, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_PI.iF.BF.norm.c)
```
...
dds.BC1_PI.iF.BF.pseudo <- normTransform(dds.BC1_PI.iF.BF)
```
...
```{r}
vsd <- vst(dds.BC1_PI.iF.BF, blind=FALSE)
# rlog transformation
rlg.dds.BC1_PI.iF.BF <- rlog(dds.BC1_PI.iF.BF, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_PI.iF.BF.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_PI.iF.BF))
...
select <- order(rowMeans(dds.BC1_PI.iF.BF.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_PI.iF.BF)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_PI.iF.BF.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_PI.iF.BF)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_PI.iF.BF.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_PI.iF.BF.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_PI.iF.BF)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_PI.iF.BF.pseudo$Group
colnames(rlg_d.Mtx) <- dds.BC1_PI.iF.BF$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_PI.iF.BF, intgroup=c("Group"), returnData=TRUE)

# Exporting the PCA data to include at as afigure in the thesis
saveRDS(pcaData, "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_ISF_PCA/BC1_PI.BF.rds")
...
colData(dds.BC1_PI.iF.BF)

# Definition of the L (LOW) group as the reference group:
dds.BC1_PI.iF.BF$Group <- relevel(dds.BC1_PI.iF.BF$Group, 
                                           ref = "L")
```
...
dds.BC1_PI.iF.BF$Backcross <- factor(dds.BC1_PI.iF.BF$Backcross)

# Update of the design:
design(dds.BC1_PI.iF.BF) <- formula(~ Group)
...
# Confirmation:
design(dds.BC1_PI.iF.BF)
```


```{r DEG_Analysis}
# Update of the 'dds' object with the DEG analysis
dds.BC1_PI.iF.BF <- DESeq(dds.BC1_PI.iF.BF)
# Results with a == 0.1
res.BC1_PI.iF.BF <- results(dds.BC1_PI.iF.BF)
# Results with a == 0.05
res.BC1_PI.iF.BF.a_0.05 <- results(dds.BC1_PI.iF.BF, alpha=0.05)
```
...
summary(res.BC1_PI.iF.BF)

# Number of significant LFCs...
sum(res.BC1_PI.iF.BF$padj < 0.1, na.rm = TRUE)

```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_PI.iF.BF.a_0.05)

# Number of significant LFCs...
sum(res.BC1_PI.iF.BF.a_0.05$padj < 0.05, na.rm = TRUE)
...
write.table(as.data.frame(na.omit(res.BC1_PI.iF.BF)), 
          file="BC1_PI.iF.BF.SF.Results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_PI.iF.BF.a_0.05 <- as.data.frame(na.omit(res.BC1_PI.iF.BF.a_0.05))

DFres.BC1_PI.iF.BF.a_0.05 <- cbind(rownames(DFres.BC1_PI.iF.BF.a_0.05), rownames(DFres.BC1_PI.iF.BF.a_0.05), DFres.BC1_PI.iF.BF.a_0.05)
DFres.BC1_PI.iF.BF.a_0.05[,1:2] <- str_split_fixed(rownames(DFres.BC1_PI.iF.BF.a_0.05),"_", n = 2) 
colnames(DFres.BC1_PI.iF.BF.a_0.05) <- c("ENSEMLB ID", "Transcript", colnames(DFres.BC1_PI.iF.BF.a_0.05)[3:length(DFres.BC1_PI.iF.BF.a_0.05)])

write.table(DFres.BC1_PI.iF.BF.a_0.05, row.names = FALSE, 
          file="BC1_PI.iF.BF.SF.Results_a_0.05.csv", sep = ";")
```
...
plotMA(res.BC1_PI.iF.BF.a_0.05)
```
...
qq.chisq(-2*log(res.BC1_PI.iF.BF$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_PI.iF.BF_above_fc.12 <- res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_PI.iF.BF_below_fc.12 <- res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_PI.iF.BF_abv.blw_fc.12 <- rbind(res.BC1_PI.iF.BF_above_fc.12, res.BC1_PI.iF.BF_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.iF.BF$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_PI.iF.BF_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.iF.BF_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.iF.BF_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_PI.iF.BF_above_fc.15 <- res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_PI.iF.BF_below_fc.15 <- res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_PI.iF.BF_abv.blw_fc.15 <- rbind(res.BC1_PI.iF.BF_above_fc.15, res.BC1_PI.iF.BF_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_PI.iF.BF$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_PI.iF.BF_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.iF.BF_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.iF.BF_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_PI.iF.BF_05above_fc.12 <- res.BC1_PI.iF.BF.a_0.05[which(res.BC1_PI.iF.BF.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_PI.iF.BF_05below_fc.12 <- res.BC1_PI.iF.BF.a_0.05[which(res.BC1_PI.iF.BF.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_PI.iF.BF_05abv.blw_fc.12 <- rbind(res.BC1_PI.iF.BF_05above_fc.12, res.BC1_PI.iF.BF_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.iF.BF.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_PI.iF.BF_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.iF.BF_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.iF.BF_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_PI.iF.BF_05above_fc.15 <- res.BC1_PI.iF.BF.a_0.05[which(res.BC1_PI.iF.BF.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_PI.iF.BF_05below_fc.15 <- res.BC1_PI.iF.BF.a_0.05[which(res.BC1_PI.iF.BF.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_PI.iF.BF_05abv.blw_fc.15 <- rbind(res.BC1_PI.iF.BF_05above_fc.15, res.BC1_PI.iF.BF_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.iF.BF.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_PI.iF.BF_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.iF.BF_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.iF.BF_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
Features <- rownames(res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$padj < 0.05),])
Features

# Specific sample (column) order: LOW before HIGH in blocks. 
samples <- rownames(colData(dds.BC1_PI.iF.BF)[order(dds.BC1_PI.iF.BF$Group, decreasing = TRUE),])

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.iF.BF)[rownames(res.BC1_PI.iF.BF[which(res.BC1_PI.iF.BF$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.iF.BF.SF_counts_Signif0.05.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_PI.iF.BF_05abv.blw_fc.15[which(res.BC1_PI.iF.BF_05abv.blw_fc.15$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.iF.BF)[rownames(res.BC1_PI.iF.BF_05abv.blw_fc.15[which(res.BC1_PI.iF.BF_05abv.blw_fc.15$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.iF.BF.SF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_PI.iF.BF_05abv.blw_fc.12[which(res.BC1_PI.iF.BF_05abv.blw_fc.12$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.iF.BF)[rownames(res.BC1_PI.iF.BF_05abv.blw_fc.12[which(res.BC1_PI.iF.BF_05abv.blw_fc.12$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.iF.BF.SF_counts_Signif0.05_FC_1.2.csv", row.names = FALSE, sep = ";")
```
