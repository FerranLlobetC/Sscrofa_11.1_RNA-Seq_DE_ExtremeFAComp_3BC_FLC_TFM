---
title: "BC1_PI.Longissimus_dorsi.R"
author: "FLC"
date: '2022-05-02'
output:
...
BC1_PI.PORK_06 <- file.path("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Genes/PK06", paste0(Animals.BC1_PI.LD$SAMPLE_NAME, ".genes.results"))
# Names of the columns
names(BC1_PI.PORK_06) <- paste0("X", Animals.BC1_PI.LD$SAMPLE_NAME) 
# The 'X' is necessary because R cannot use column names staring whit a number.

### IMPORT
tx.BC1_PI.LD <- tximport(BC1_PI.PORK_06, type ="rsem", txIn = FALSE, txOut = FALSE)

# Header of the imported counts
head(tx.BC1_PI.LD$counts)
...
tx.BC1_PI.LD$length[tx.BC1_PI.LD$length == 0] <- 1
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_PI.LD <- DESeqDataSetFromTximport(tx.BC1_PI.LD,
...
CMX_dds.BC1_PI.LD <- counts(dds.BC1_PI.LD)
CMX_dds.BC1_PI.LD <- CMX_dds.BC1_PI.LD[,rownames(colData(dds.BC1_PI.LD)[order(colData(dds.BC1_PI.LD)[,8]),])]
# Replace of the 'X'
colnames(CMX_dds.BC1_PI.LD) <- str_replace(colnames(CMX_dds.BC1_PI.LD),"X","")
CMX_dds.BC1_PI.LD <- cbind(rownames(CMX_dds.BC1_PI.LD), CMX_dds.BC1_PI.LD) 
colnames(CMX_dds.BC1_PI.LD) <- c("Feature", colnames(CMX_dds.BC1_PI.LD)[2:dim(CMX_dds.BC1_PI.LD)[2]])
write.table(CMX_dds.BC1_PI.LD, file = "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/COUNTS.BC1_PI.LD.csv", sep =";", row.names = FALSE)
################################################################################
# Removal of unutilized objects
rm(tx.BC1_PI.LD)
```
...
write.table(as.data.frame(na.omit(res.BC1_PI.LD)), 
          file="BC1_PI.LD.Results_a_0.1.csv", sep = ";")
...
write.table(DFres.BC1_PI.LD.a_0.05, row.names = FALSE, 
          file="BC1_PI.LD.Results_a_0.05.csv", sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_PI.LD_counts_Signif0.05.csv", row.names = FALSE, sep = ";")
```
...
# Wiritn of the csv
write.table(df, "BC1_PI.LD_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
```{r}
