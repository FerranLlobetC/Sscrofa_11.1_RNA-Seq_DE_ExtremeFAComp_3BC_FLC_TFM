...
# Liver
Animals.BC1_PI.LD <- Animals.BC1_PI[which(Animals.BC1_PI[,4] == "Longissimusdorsi"),]

Animals.BC1_PI.LD <- Animals.BC1_PI.LD[order(Animals.BC1_PI.LD[,1]),]
...
cnt.BC1_PI.LD <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/COUNTS.BC1_PI.LD.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_PI.LD) <- cnt.BC1_PI.LD[,1]

cnt.BC1_PI.LD <- cnt.BC1_PI.LD[,2:length(cnt.BC1_PI.LD)]

cnt.BC1_PI.LD <- cnt.BC1_PI.LD[,order(colnames(cnt.BC1_PI.LD))]
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_PI.LD <- DESeqDataSetFromMatrix(countData = cnt.BC1_PI.LD,
                              colData = Animals.BC1_PI.LD,
                              design = ~ Group) # This design is temporary.

# Header of the counts now converted into postive integers: 
head(counts(dds.BC1_PI.LD)) # This counts are unormalized. 
...
```{r pre_filtering}
# Number of samples
N <- nrow(Animals.BC1_PI.LD)
# Features to keep
keep <- rowSums(counts(dds.BC1_PI.LD)) >= N

a <- dim(dds.BC1_PI.LD)[1]
# Application of the pre-filtering over the dds object. 
dds.BC1_PI.LD <- dds.BC1_PI.LD[keep,]

(dim(dds.BC1_PI.LD)[1] - a) / a
################################################################################
# Removal of unutilized objects
#rm(keep)
```
...
# creates 'sj' or 'sij' 
dds.BC1_PI.LD <- estimateSizeFactors(dds.BC1_PI.LD)

# DESeq2 can use 2 Normalization constants: sj & sij
# 'sj' 
head(sizeFactors(dds.BC1_PI.LD)) 
# It is empty because gene-specific normalization factors. Why? I belive it is 
# linked to the fact we use RSEM for the counts.

# 'sij' 
head(normalizationFactors(dds.BC1_PI.LD))

### Normalization using above sij within the means (muij) of each gene-sample.
dds.BC1_PI.LD.norm.c <- counts(dds.BC1_PI.LD, normalized = TRUE) # This parameter is what normalized the count data using the above sij factors. 
print("Normalized counts")
head(dds.BC1_PI.LD.norm.c)
```
...
dds.BC1_PI.LD.pseudo <- normTransform(dds.BC1_PI.LD)
```
...
```{r}
vsd <- vst(dds.BC1_PI.LD, blind=FALSE)
# rlog transformation
rlg.dds.BC1_PI.LD <- rlog(dds.BC1_PI.LD, blind=FALSE)
```
...
meanSdPlot(assay(dds.BC1_PI.LD.pseudo))

# Mean vs SD plot of the rlog transformed counts
meanSdPlot(assay(rlg.dds.BC1_PI.LD))
...
select <- order(rowMeans(dds.BC1_PI.LD.norm.c),
                decreasing=TRUE)[1:20]
df <- as.data.frame(colData(dds.BC1_PI.LD)[,c("Sex","Group")])

# Heat map of the normalized pseudocounts
pheatmap(assay(dds.BC1_PI.LD.pseudo)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)

# Heat map of the rlog transformed counts
pheatmap(assay(rlg.dds.BC1_PI.LD)[select,], cluster_rows=FALSE, show_rownames=TRUE,
         cluster_cols=FALSE, annotation_col=df)
...
pseudo_dist <- dist(t(assay(dds.BC1_PI.LD.pseudo)))
# Conversion into a matrix
pseudo_d.Mtx <- as.matrix(pseudo_dist)

# Addition of the GROUP information
rownames(pseudo_d.Mtx) <- dds.BC1_PI.LD.pseudo$Group
...
# Computation of the counts Eculidean distances
rlg_dist <- dist(t(assay(rlg.dds.BC1_PI.LD)))
# Conversion into a matrix
rlg_d.Mtx <- as.matrix(rlg_dist)

# Addition of the SEX & GROUP information
rownames(rlg_d.Mtx) <- dds.BC1_PI.LD.pseudo$Group
colnames(rlg_d.Mtx) <- dds.BC1_PI.LD$SAMPLE_NAME
...
pcaData <- plotPCA(rlg.dds.BC1_PI.LD, intgroup=c("Group"), returnData=TRUE)

# Saving the pcaData object for the thesis figures
saveRDS(pcaData, file = "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_PCA/BC1_PI.LD.rds")
...
colData(dds.BC1_PI.LD)

# Definition of the L (LOW) group as the reference group:
dds.BC1_PI.LD$Group <- relevel(dds.BC1_PI.LD$Group, 
                                           ref = "L")
```
...
dds.BC1_PI.LD$Backcross <- factor(dds.BC1_PI.LD$Backcross)

# Update of the design:
design(dds.BC1_PI.LD) <- formula(~ Group)
...
# Confirmation:
design(dds.BC1_PI.LD)
```


```{r DEG_Analysis}
# Update of the 'dds' object with the DEG analysis
dds.BC1_PI.LD <- DESeq(dds.BC1_PI.LD)
# Results with a == 0.1
res.BC1_PI.LD <- results(dds.BC1_PI.LD)
# Results with a == 0.05
res.BC1_PI.LD.a_0.05 <- results(dds.BC1_PI.LD, alpha=0.05)
```
...
summary(res.BC1_PI.LD)

# Number of significant LFCs...
sum(res.BC1_PI.LD$padj < 0.1, na.rm = TRUE)

# Visualization of the significant LFCs...
resV1 <- res.BC1_PI.LD[which(res.BC1_PI.LD$padj < 0.1),]
resV1[which(abs(resV1$log2FoldChange) > 1.5), ]
```

```{r DEGA_results_embellishment_a_0.05}
# Summary of the up and down regulated genes in this study:
summary(res.BC1_PI.LD.a_0.05)

# Number of significant LFCs...
sum(res.BC1_PI.LD.a_0.05$padj < 0.05, na.rm = TRUE)

# Visualization of the significant LFCs...
#res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$padj < 0.05),]

resV2 <- res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$padj < 0.1),]
...
write.table(as.data.frame(na.omit(res.BC1_PI.LD)), 
          file="BC1_PI.LD.SF.Results_a_0.1.csv", sep = ";")

# alpha == 0.05 in the independent filtering 
DFres.BC1_PI.LD.a_0.05 <- as.data.frame(na.omit(res.BC1_PI.LD.a_0.05))

DFres.BC1_PI.LD.a_0.05 <- cbind(rownames(DFres.BC1_PI.LD.a_0.05), DFres.BC1_PI.LD.a_0.05)
colnames(DFres.BC1_PI.LD.a_0.05) <- c("feature", colnames(DFres.BC1_PI.LD.a_0.05)[2:length(DFres.BC1_PI.LD.a_0.05)])

write.table(DFres.BC1_PI.LD.a_0.05, row.names = FALSE, 
          file="BC1_PI.LD.SF.Results_a_0.05.csv", sep = ";")
```
...
qq.chisq(-2*log(res.BC1_PI.LD$pvalue),df=2,pvals=T,overdisp=T, 
...
# Higher
res.BC1_PI.LD_above_fc.12 <- res.BC1_PI.LD[which(res.BC1_PI.LD$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_PI.LD_below_fc.12 <- res.BC1_PI.LD[which(res.BC1_PI.LD$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_PI.LD_abv.blw_fc.12 <- rbind(res.BC1_PI.LD_above_fc.12, res.BC1_PI.LD_below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_PI.LD_abv.blw_fc.12$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.LD_above_fc.12$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.LD_below_fc.12$padj < 0.1, na.rm = TRUE)`
```{r FC_1.5+a_0.1}
# FC = 1.5
lg2_fc1.5 <- log2(1.5)
...
# Higher
res.BC1_PI.LD_above_fc.15 <- res.BC1_PI.LD[which(res.BC1_PI.LD$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_PI.LD_below_fc.15 <- res.BC1_PI.LD[which(res.BC1_PI.LD$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_PI.LD_abv.blw_fc.15 <- rbind(res.BC1_PI.LD_above_fc.15, res.BC1_PI.LD_below_fc.15)
```

The number of significant genes get reduced from `r sum(res.BC1_PI.LD$padj < 0.1, na.rm = TRUE)` to `r sum(res.BC1_PI.LD_abv.blw_fc.15$padj < 0.1, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.LD_above_fc.15$padj < 0.1, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.LD_below_fc.15$padj < 0.1, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.2:
# Higher
res.BC1_PI.LD_05above_fc.12 <- res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$log2FoldChange >= lg2_fc1.2),]
# Lower 
res.BC1_PI.LD_05below_fc.12 <- res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$log2FoldChange <= -lg2_fc1.2),]
# Merge
res.BC1_PI.LD_05abv.blw_fc.12 <- rbind(res.BC1_PI.LD_05above_fc.12, res.BC1_PI.LD_05below_fc.12)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_PI.LD_05abv.blw_fc.12$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.LD_05above_fc.12$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.LD_05below_fc.12$padj < 0.05, na.rm = TRUE)`
...
# Selection of the features with a log2 FC higher or lower than 1.5:
# Higher
res.BC1_PI.LD_05above_fc.15 <- res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$log2FoldChange >= lg2_fc1.5),]
# Lower 
res.BC1_PI.LD_05below_fc.15 <- res.BC1_PI.LD.a_0.05[which(res.BC1_PI.LD.a_0.05$log2FoldChange <= -lg2_fc1.5),]
# Merge
res.BC1_PI.LD_05abv.blw_fc.15 <- rbind(res.BC1_PI.LD_05above_fc.15, res.BC1_PI.LD_05below_fc.15)
```

The number of significant genes gets reduced from `r sum(res.BC1_PI.LD.a_0.05$padj < 0.05, na.rm = TRUE)` to `r sum(res.BC1_PI.LD_05abv.blw_fc.15$padj < 0.05, na.rm = TRUE)`.

Number of up-regulated genes: `r sum(res.BC1_PI.LD_05above_fc.15$padj < 0.05, na.rm = TRUE)`
Number of down-regulated genes: `r sum(res.BC1_PI.LD_05below_fc.15$padj < 0.05, na.rm = TRUE)`
...
Features <- rownames(res.BC1_PI.LD[which(res.BC1_PI.LD$padj < 0.05),])
Features

# Specific sample (column) order: LOW before HIGH in blocks. 
samples <- rownames(colData(dds.BC1_PI.LD)[order(dds.BC1_PI.LD$Group, decreasing = TRUE),])

# Generation of the dataframe
# 1 (LEFT): Selection of just the signficative + 1.5 FC features 
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.LD)[rownames(res.BC1_PI.LD[which(res.BC1_PI.LD$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.LD.SF_counts_Signif0.05.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_PI.LD_05abv.blw_fc.15[which(res.BC1_PI.LD_05abv.blw_fc.15$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.LD)[rownames(res.BC1_PI.LD_05abv.blw_fc.15[which(res.BC1_PI.LD_05abv.blw_fc.15$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.LD.SF_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
...
# Same feature name as the RSEM counterpart to ease the 'BUSCARV' strategy
Features <- rownames(res.BC1_PI.LD_05abv.blw_fc.12[which(res.BC1_PI.LD_05abv.blw_fc.12$padj < 0.05),])
Features
...
# rownames + which)
# 2 (RIGHT): Order of the samples
df <- counts(dds.BC1_PI.LD)[rownames(res.BC1_PI.LD_05abv.blw_fc.12[which(res.BC1_PI.LD_05abv.blw_fc.12$padj < 0.05),]), samples]
...
# Wiritn of the csv
write.table(df, "BC1_PI.LD.SF_counts_Signif0.05_FC_1.2.csv", row.names = FALSE, sep = ";")
```


## Addition of NCBI Anotation
I include the sus scfora NCBI annotation data to inlcude the full-lengh gene names to enrich the infomration of the results CSV files. 
```{r}
# Loading of the genomic features using the wonderful package-data-set 'org.Ss.eg.db' 

# List of columns of the 'tx' object:
columns(org.Ss.eg.db)  # FROM NCBI - Until now I used the annotations from ENSEMBL

# Keys of the 'tx' object. What we can search for, I think. 
keytypes(org.Ss.eg.db)

# GENE SYMBOLS of the PIG GENES
SS.11.genes <- keys(org.Ss.eg.db, "SYMBOL")


# Selection of the columns "GENENAME" to add it to my output files 
SS.11.gene_data <- select(org.Ss.eg.db, SS.11.genes, columns = c("SYMBOL", "GENENAME"), keytype = "SYMBOL")
write.table(SS.11.gene_data, "/home/fllobet/Documents/RNA-Seq_TFM/gene_data.csv", sep = ";")
```
