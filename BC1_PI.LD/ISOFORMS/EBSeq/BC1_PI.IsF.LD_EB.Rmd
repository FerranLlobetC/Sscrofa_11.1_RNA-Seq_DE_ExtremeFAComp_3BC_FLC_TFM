...
Code used in bash to obtain the `RSEM_counts_BC1_PI.LD.matrix` in the workstation:
```{comment}
conda activate RNA-Seq
./scripts/RSEM_Matrix.sh 0607_CORRECTSampleNames.csv "," "Backcross-BC1_PI,Tissue-Longissimusdorsi" isoforms ./scripts/ 7 ./COUNTS/ RSEM_counts_BC1_PI.LD
workie -f RNA-Seq/RNA-Seq_11.1_FLC/RSEM_counts_BC1_PI.LD.matrix
```

# Lodaing *expected counts* from RSEM
```{r rsem_Counts_LOADING}
RSEM_exCnt.BC1_PI.iF.LD <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/RSEM_counts_BC1_PI.LD.matrix", sep = "\t", row.names = 1)
# Modification of the colnames:
colnames(RSEM_exCnt.BC1_PI.iF.LD) <- str_replace(str_replace(colnames(RSEM_exCnt.BC1_PI.iF.LD),".isoforms.results",""),"^[.-_]+\\.","X")
View(RSEM_exCnt.BC1_PI.iF.LD)
```
...
RSEM_exCnt.BC1_PI.iF.LD <- RSEM_exCnt.BC1_PI.iF.LD[,rownames(Animals.BC1_PI.iF.LD)]


# Cofnirmation that both namesets match
colnames(RSEM_exCnt.BC1_PI.iF.LD) == rownames(Animals.BC1_PI.iF.LD)
################################################################################
# Removal of unutilized objects
rm(Animals)
```
The group vector will be `r Animals.BC1_PI.iF.LD$Group`

# Isoform and gene names
```{r isoform_names}
iso_names <- rownames(RSEM_exCnt.BC1_PI.iF.LD)
```

```{r gene_names}
# The first 'isofroms.results' file of the data will be used to retrieve the gene names
file <- paste0("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK06/",str_replace(colnames(RSEM_exCnt.BC1_PI.iF.LD)[1],"X",""),".isoforms.results")
gene_names <- read.delim(file)$gene_id
```

# Normalization: Library Size Factor
```{r library_size}
libSzF.BC1_PI.iF.LD <- MedianNorm(RSEM_exCnt.BC1_PI.iF.LD)
```
...
```{r Ng_vector}
Ng_BC1_PI.iF.LD <- GetNg(IsoformName = iso_names, GeneName = gene_names)
# The ACTUAL Ng vector that will be feed to the EBSeq model fitting
NgV_BC1.Pi.iF.LD <- Ng_BC1_PI.iF.LD$IsoformNgTrun
...
groups <- relevel(as.factor(Animals.BC1_PI.iF.LD$Group), ref = "L")

# Repeating the fitting but using n = 10 iterations because the 3 above 
# parameters did not stabilize changing les e-3 = 0.01:
# Model fiting
EB.BC1_PI.iF.LD <- EBTest(Data=as.matrix(RSEM_exCnt.BC1_PI.iF.LD), 
                          NgVector=NgV_BC1.Pi.iF.LD, 
                          Conditions=groups,
                          sizeFactors=as.vector(libSzF.BC1_PI.iF.LD), 
                          maxround=15)

EB.BC1_PI.iF.LD$Alpha
EB.BC1_PI.iF.LD$Beta
EB.BC1_PI.iF.LD$P
```
...
robust.EBRes.BC1_PI.iF.LD <- GetDEResults(EB.BC1_PI.iF.LD, FDR = 0.05, Method = "robust")

# Aoplying the classic method
classic.EBRes.BC1_PI.iF.LD <- GetDEResults(EB.BC1_PI.iF.LD, FDR = 0.05, Method = "classic")

# Nº of DE Isoforoms 
# Robust method
length(robust.EBRes.BC1_PI.iF.LD$DEfound)
# Classic method
length(classic.EBRes.BC1_PI.iF.LD$DEfound)
```
# Calculation of Fold-Changes
```{r FC}
FC.BC1_PI.iF.LD <- PostFC(EB.BC1_PI.iF.LD)
FC.BC1_PI.iF.LD <- FC.BC1_PI.iF.LD$RealFC
# TO GET NUMBER OF FILTERED ISOFOROMS
length(FC.BC1_PI.iF.LD)
length(classic.EBRes.BC1_PI.iF.LD$PPMat)

# Generation of a results matrix of the classic method:
classicalFC.PP_matrix <- cbind(names(FC.BC1_PI.iF.LD), names(FC.BC1_PI.iF.LD), FC.BC1_PI.iF.LD, -log2(FC.BC1_PI.iF.LD), classic.EBRes.BC1_PI.iF.LD$PPMat[which(rownames(classic.EBRes.BC1_PI.iF.LD$PPMat) %in% names(FC.BC1_PI.iF.LD)),])
...
write.table(classicalFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/EBSeq/BC1_PI.iF.LD_EBcl.Results.csv", sep = ";", row.names = FALSE)
# Results CSV of only the significant features
write.table(classicalFC.PP_matrix[which(rownames(classicalFC.PP_matrix) %in% classic.EBRes.BC1_PI.iF.LD$DEfound),], "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/EBSeq/BC1_PI.iF.LD_EBcl_SIGNIF.Results.csv", sep = ";", row.names = FALSE)


# Generation of a results matrix of the robust method:
robustFC.PP_matrix <- cbind(names(FC.BC1_PI.iF.LD), names(FC.BC1_PI.iF.LD), FC.BC1_PI.iF.LD, -log2(FC.BC1_PI.iF.LD), robust.EBRes.BC1_PI.iF.LD$PPMat[which(rownames(robust.EBRes.BC1_PI.iF.LD$PPMat) %in% names(FC.BC1_PI.iF.LD)),])
...
# Results CSV of all the features
write.table(robustFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/EBSeq/BC1_PI.iF.LD_EBrb.Results.csv", sep = ";", row.names = FALSE)

# Results CSV of only the significant features
write.table(robustFC.PP_matrix[which(rownames(robustFC.PP_matrix) %in% robust.EBRes.BC1_PI.iF.LD$DEfound),], "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/EBSeq/BC1_PI.iF.LD_EBrb_SIGNIF.Results.csv", sep = ";", row.names = FALSE)

# Results CSV of only the significant features
```
# Calculation of percentage of filtered features
```{r filtered_features}
(dim(RSEM_exCnt.BC1_PI.iF.LD)[1] - dim(EB.BC1_PI.iF.LD$PPMat)[1]) / dim(RSEM_exCnt.BC1_PI.iF.LD)[1]
...
robustFC_signif <- robustFC.PP_matrix[which(rownames(robustFC.PP_matrix) %in% robust.EBRes.BC1_PI.iF.LD$DEfound),]
...
```{r comp_DESeq2}
BC1_PI.iF.LD_Dq2 <- read.csv("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/BC1_PI.iF.LD.SF.Results_a_0.05.csv", sep = ";")

# Selection of just the significant faetures
BC1_PI.iF.LD_Dq2 <- BC1_PI.iF.LD_Dq2[which(BC1_PI.iF.LD_Dq2[,8] < 0.05),]

# Row names of both objects become the ENSMEBL ID
rownames(BC1_PI.iF.LD_Dq2) <- BC1_PI.iF.LD_Dq2$ENSEMLB.ID
rownames(robustFC_signif) <- robustFC_signif[,1]
# 'BUSCARV' between the 2 lists
Common <- merge(robustFC_signif, BC1_PI.iF.LD_Dq2, by = 0)
...
nrow(Common)/nrow(BC1_PI.iF.LD_Dq2)
...
write.table(robustFC_signif, "BC1_PI.iF.LD_EBrb_SIGNIF.Results.csv", row.names = FALSE, sep = ";")
```
...
signifRSEMCnts <- as.data.frame(RSEM_exCnt.BC1_PI.iF.LD[,
                                          colnames(RSEM_exCnt.BC1_PI.iF.LD[
                                            order(Animals.BC1_PI.iF.LD$Group, 
                                                  decreasing = TRUE)])])
...
write.table(signifRSEMCnts, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LD/ISOFORMS/EBSeq/BC1_PI.iF.LD_EBrb_COUNTS.csv", sep = ";", 
            row.names = F)
```
