...
cnt.BC1_PI.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/HTSeq/cnts.csv", sep=";", row.names = 1)
# It is important to state that the row names are the 1st column. 
rownames(cnt.BC1_PI.LV) <- str_c(rownames(cnt.BC1_PI.LV), cnt.BC1_PI.LV[,1], cnt.BC1_PI.LV[,2], sep = "_")
# Also the gene name and chromosome colmuns should be removed.
cnt.BC1_PI.LV <- cnt.BC1_PI.LV[,3:length(cnt.BC1_PI.LV)]

# Extra 'HTSeq' information rows (lsat 5 rows) should be removed too:
rws <- nrow(cnt.BC1_PI.LV)[1]-5
cnt.BC1_PI.LV <- cnt.BC1_PI.LV[1:rws,]
...
write.table(DFres.BC1_PI.LV.a_0.05, row.names = FALSE, 
          file="BC1_PI.LV.HTSEQ_DEGanalysis_results_a_0.05.csv", sep = ";")
```
...
Features <- str_split_fixed(rownames(res.BC1_PI.LV_abv.blw_fc.15[which(res.BC1_PI.LV_05abv.blw_fc.15$padj < 0.05),]),"_", 3)[,1:2]
...
# Wiritn of the csv
write.table(df, "BC1_PI.LV_HTSeq_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
