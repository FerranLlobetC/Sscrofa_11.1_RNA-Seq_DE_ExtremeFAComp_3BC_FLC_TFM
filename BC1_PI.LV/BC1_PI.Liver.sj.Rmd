...
These counts were first lodaded using `tximport` from `RSEM` output files.

```{r tximport_RSEM_counts_loading}
cnt.BC1_PI.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/IntegerCOUNTS.BC1_PI.liver.csv", sep = ";")

# Feature ID as row name
rownames(cnt.BC1_PI.LV) <- cnt.BC1_PI.LV[,1]

cnt.BC1_PI.LV <- cnt.BC1_PI.LV[,2:length(cnt.BC1_PI.LV)]
...
```{r dds_object}
### Standard 'dds' object:
dds.BC1_PI.LV <- DESeqDataSetFromMatrix(countData = cnt.BC1_PI.LV,
...
saveRDS(pcaData, file = "~/Documents/RNA-Seq_TFM/Extra_figures/dq2_PCA/BC1_PI.LV.rds")
...
sum(mcols(dds.BC1_PI.LV,use.names=TRUE)[,9] == TRUE)
```
...
rownames(res.BC1_PI.LV.a_0.05[which(res.BC1_PI.LV.a_0.05$padj < 0.05),])
```
...
write.table(DFres.BC1_PI.LV.a_0.05, row.names = FALSE, 
          file="BC1_PI.LV.SF.Results_a_0.05.csv", sep = ";")
```
...
```
# ggplot2 Implementation of previous plots
```{r}
# Generation of a new DF with a column determing wheter a feature
# passed or not filtering + number of each type
# PASS or NOT
IF_BC1_PI.LV_05 <- cbind(
  as.data.frame(res.BC1_PI.LV.a_0.05),
  ifelse(res.BC1_PI.LV.a_0.05$baseMean > 
           metadata(res.BC1_PI.LV.a_0.05)$filterThreshold, "PASS", "NOT pass"))

# Colnames update
colnames(IF_BC1_PI.LV_05) <- c(colnames(IF_BC1_PI.LV_05)[1:ncol(IF_BC1_PI.LV_05)-1], "IF_Pass")
# Factor reorder so it its NOT -> YEP
IF_BC1_PI.LV_05$IF_Pass <- relevel(as.factor(IF_BC1_PI.LV_05$IF_Pass), ref = "PASS")


########################## pvalues + threshold #################################


Pv_mean <- ggplot(IF_BC1_PI.LV_05, aes(x = baseMean+1, y = -log10(pvalue))) + geom_jitter(color = "#4d5d53", alpha = 0.65) +
  geom_hline(yintercept = -log10(0.05), color = "#2f847c", linetype = "dashed",
            size = 1) +
  geom_vline(xintercept = metadata(res.BC1_PI.LV.a_0.05)$filterThreshold, 
             color = "#bdb76b", size = 1,  linetype = "dashed") +
  scale_x_continuous(expand = c(0.01,0), limits = c(0, 450000)) +
  labs(x = "Mean counts", y = "-log<sub>10</sub>(p-value)", title = "a)") +
  theme_bw() +
  theme(text = element_text(family = "ubuntu", face = "bold"), axis.title.y = element_markdown())


IF_hist <- ggplot(data = IF_BC1_PI.LV_05[IF_BC1_PI.LV_05$pvalue < 1,], aes(x = pvalue, fill = IF_Pass)) +
  geom_bar(stat = "bin", col = "#4d5d53", alpha = 0.65) +
  geom_vline(xintercept = 0.051, linetype = "dashed", size = 2, color = "#2f847c") + 
  geom_text(aes(x = 0.055, y = 700), label = "0.05", 
            color = "#2f847c", size = 4, nudge_x = 0.02,
            fontface = "bold") +
  scale_fill_manual(values = c("#bdb76b", "#ace1af")) + 
  scale_y_continuous(expand = c(0,0)) +
  scale_x_continuous(expand = c(0, 0), 
                     limits = c(0,1), breaks = seq(0,1,0.1)) +
  labs(x = "Wald test p-value distribution (0 ~ 1)",
       y = "Frequency (number of genomic features)",
       title = "b)") +
  theme_bw() + 
  theme(text = element_text(family = "ubuntu", face = "bold"),
                     legend.position = "none")

plts <- ggarrange(Pv_mean, IF_hist)


ggsave("~/Documents/RNA-Seq_TFM/Extra_figures/IFBC1_PI.LV.jpg", 
       plot = plts, height = 7.74, width = 17.8, dpi = "retina")
```
...
Features <- rownames(res.BC1_PI.LV_abv.blw_fc.15[which(res.BC1_PI.LV_05abv.blw_fc.15$padj < 0.05),])
Features
...
df <- cnt.BC1_PI.LV[rownames(res.BC1_PI.LV_abv.blw_fc.15[which(res.BC1_PI.LV_05abv.blw_fc.15$padj < 0.05),]), paste0("X",samples)]
...
# Wiritn of the csv
write.table(df, "BC1_PI.LV_JUST.sj_counts_Signif0.05_FC_1.5.csv", row.names = FALSE, sep = ";")
```
