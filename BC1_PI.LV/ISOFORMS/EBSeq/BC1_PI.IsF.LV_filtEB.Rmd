...
RSEM_exCnt.BC1_PI.iF.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/RSEM_counts_BC1.PI.LV.matrix", sep = "\t", row.names = 1)
# Modification of the colnames:
colnames(RSEM_exCnt.BC1_PI.iF.LV) <- str_replace(str_replace(colnames(RSEM_exCnt.BC1_PI.iF.LV),".isoforms.results",""),"COUNTS.","X")
View(RSEM_exCnt.BC1_PI.iF.LV)
```
...
RSEM_exCnt.BC1_PI.iF.LV <- RSEM_exCnt.BC1_PI.iF.LV[,rownames(Animals.BC1_PI.iF.LV)]

################################################################################
# Removal of unutilized objects
rm(Animals)
```
The group vector will be `r Animals.BC1_PI.iF.LV$Group`

# Isoform and gene names
```{r isoform_names}
iso_names <- rownames(RSEM_exCnt.BC1_PI.iF.LV)
```

```{r gene_names}
# The first 'isofroms.results' file of the data will be used to retrieve the gene names
file <- paste0("/home/fllobet/Documents/RNA-Seq_TFM/COUNTS/Isoforms/PK06/",str_replace(colnames(RSEM_exCnt.BC1_PI.iF.LV)[1],"X",""),".isoforms.results")
gene_names <- read.delim(file)$gene_id
```

# Pre-filtering
The same approach than in DESeq2 is applied
```{r pre-filtering}
# Filtering of the data matrix
N <- ncol(RSEM_exCnt.BC1_PI.iF.LV)
keep <- rowSums(RSEM_exCnt.BC1_PI.iF.LV) > N
RSEM_exCnt.BC1_PI.iF.LV <- RSEM_exCnt.BC1_PI.iF.LV[keep,]

# Update of isoform names and gene names
iso_names <- iso_names[keep]
gene_names <- gene_names [keep]
```

# Normalization: Library Size Factor
```{r library_size}
libSzF.BC1_PI.iF.LV <- MedianNorm(RSEM_exCnt.BC1_PI.iF.LV)
```
...
```{r Ng_vector}
Ng_BC1_PI.iF.LV <- GetNg(IsoformName = iso_names, GeneName = gene_names)
# The ACTUAL Ng vector that will be feed to the EBSeq model fitting
NgV_BC1.Pi.iF.LV <- Ng_BC1_PI.iF.LV$IsoformNgTrun
...
groups <- relevel(as.factor(Animals.BC1_PI.iF.LV$Group), ref = "L")

# Repeating the fitting but using n = 10 iterations because the 3 above 
# parameters did not stabilize changing les e-3 = 0.01:
# Model fiting
EB.BC1_PI.iF.LV <- EBTest(Data=as.matrix(RSEM_exCnt.BC1_PI.iF.LV), 
                          NgVector=NgV_BC1.Pi.iF.LV, 
                          Conditions=groups,
                          sizeFactors=as.vector(libSzF.BC1_PI.iF.LV), 
                          maxround=15)

EB.BC1_PI.iF.LV$Alpha
EB.BC1_PI.iF.LV$Beta
EB.BC1_PI.iF.LV$P
```
...
robust.EBRes.BC1_PI.iF.LV <- GetDEResults(EB.BC1_PI.iF.LV, FDR = 0.05, Method = "robust")

# Aoplying the classic method
classic.EBRes.BC1_PI.iF.LV <- GetDEResults(EB.BC1_PI.iF.LV, FDR = 0.05, Method = "classic")

# Nº of DE Isoforoms 
# Robust method
length(robust.EBRes.BC1_PI.iF.LV$DEfound)
# Classic method
length(classic.EBRes.BC1_PI.iF.LV$DEfound)
```
# Calculation of Fold-Changes
```{r FC}
FC.BC1_PI.iF.LV <- PostFC(EB.BC1_PI.iF.LV)
FC.BC1_PI.iF.LV <- FC.BC1_PI.iF.LV$RealFC
# TO GET NUMBER OF FILTERED ISOFOROMS
length(FC.BC1_PI.iF.LV)
length(classic.EBRes.BC1_PI.iF.LV$PPMat)
classic.EBRes.BC1_PI.iF.LV$DEfound

# Generation of a results matrix of the classic method:
classicalFC.PP_matrix <- cbind(names(FC.BC1_PI.iF.LV), names(FC.BC1_PI.iF.LV), FC.BC1_PI.iF.LV, -log2(FC.BC1_PI.iF.LV), classic.EBRes.BC1_PI.iF.LV$PPMat[which(rownames(classic.EBRes.BC1_PI.iF.LV$PPMat) %in% names(FC.BC1_PI.iF.LV)),])
...
write.table(classicalFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/ISOFORMS/EBSeq/BC1_PI.iF.LV_filtEBcl.Results.csv", sep = ";")
write.table(classicalFC.PP_matrix[which(rownames(classicalFC.PP_matrix) %in% classic.EBRes.BC1_PI.iF.LV$DEfound),], "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/ISOFORMS/EBSeq/BC1_PI.iF.LV_filtEBcl.Results.csv", sep = ";")

# Generation of a results matrix of the robust method:
robustFC.PP_matrix <- cbind(names(FC.BC1_PI.iF.LV), names(FC.BC1_PI.iF.LV), FC.BC1_PI.iF.LV, -log2(FC.BC1_PI.iF.LV), robust.EBRes.BC1_PI.iF.LV$PPMat[which(rownames(robust.EBRes.BC1_PI.iF.LV$PPMat) %in% names(FC.BC1_PI.iF.LV)),])
...
write.table(robustFC.PP_matrix, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/ISOFORMS/EBSeq/BC1_PI.iF.LV_filtEBrb.Results.csv", sep = ";")
```

# Calculation of percentage of filtered features
```{r filtered_features}
(dim(RSEM_exCnt.BC1_PI.iF.LV)[1] - dim(EB.BC1_PI.iF.LV$PPMat)[1]) / dim(RSEM_exCnt.BC1_PI.iF.LV)[1]
...
robustFC_signif <- robustFC.PP_matrix[which(rownames(robustFC.PP_matrix) %in% robust.EBRes.BC1_PI.iF.LV$DEfound),]
...
```{r comp_DESeq2}
BC1_PI.iF.LV_Dq2 <- read.csv("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/ISOFORMS/BC1_PI.iF.LV.SF.Results_a_0.05.csv", sep = ";")

# Selection of just the significant faetures
BC1_PI.iF.LV_Dq2 <- BC1_PI.iF.LV_Dq2[which(BC1_PI.iF.LV_Dq2[,7] < 0.05),]

# Row names of both objects become the ENSMEBL ID
rownames(BC1_PI.iF.LV_Dq2) <- BC1_PI.iF.LV_Dq2$ENSEMLB.ID
rownames(robustFC_signif) <- robustFC_signif[,1]
# 'BUSCARV' between the 2 lists
Common <- merge(robustFC_signif, BC1_PI.iF.LV_Dq2, by = 0)
...
nrow(Common)/nrow(BC1_PI.iF.LV_Dq2)
...
write.table(robustFC_signif, "BC1_PI.iF_LV_filtEBrb_SIGNIF.Results.csv", row.names = FALSE, sep = ";")
```
...
signifRSEMCnts <- as.data.frame(RSEM_exCnt.BC1_PI.iF.LV[,
                                          colnames(RSEM_exCnt.BC1_PI.iF.LV[
                                            order(Animals.BC1_PI.iF.LV$Group, 
                                                  decreasing = TRUE)])])
...
write.table(signifRSEMCnts, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/ISOFORMS/EBSeq/BC1_PI.iF.LV_EBrb_COUNTS.csv", sep = ";", 
            row.names = F)
```
