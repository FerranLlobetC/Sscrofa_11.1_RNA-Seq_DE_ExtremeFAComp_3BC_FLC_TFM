...
# Selection of 'BC1_LD'
Animals.BC1_LD <- Animals[which(Animals[,2] == "BC1_PI"),]

# Selection of 'liver'
Animals.BC1_PI.LV <- Animals.BC1_LD[which(Animals.BC1_LD[,3] == "Liver"),]
```
...
```{r load_counts}
cnt.BC1_PI.LV <- read.delim("/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/IntegerCOUNTS.BC1_PI.liver.csv", sep = ";", row.names = 1)

# Conversion into a matrix
cnt.BC1_PI.LV <- as.matrix(cnt.BC1_PI.LV)
```

# Creation of a `DGEList` **object**
Like in `desEq2` where a `dds` object had to be created here a `DGEList` object must be created.
```{r DGEList_object}
compare <- relevel(as.factor(Animals.BC1_PI.LV$Group), ref = "L")

################ CHECKING COLS count matrix == ROWS metadata ###################
# The while loop serves to see the OK prompt when the orders has been modified. 
while (TRUE) {
  if (levels(as.factor(rownames(Animals.BC1_PI.LV) == 
                       colnames(cnt.BC1_PI.LV))) == TRUE) {
  ...
  cnt.BC1_PI.LV <- cnt.BC1_PI.LV[,rownames(Animals.BC1_PI.LV)]
}
}

dge.BC1_PI.LV <- DGEList(cnt.BC1_PI.LV, group = compare)
dge.BC1_PI.LV
```
...
```{r filtering}
a <- dim(dge.BC1_PI.LV$counts)[1]

# Filtering
keep <- filterByExpr(dge.BC1_PI.LV)
dge.BC1_PI.LV <- dge.BC1_PI.LV[keep, , keep.lib.sizes=FALSE]

# Percentage of remove features
round(((dim(dge.BC1_PI.LV$counts)[1] - a) / a), 4)

# Re-computation of the library sizes
dge.BC1_PI.LV$samples$lib.size <- colSums(dge.BC1_PI.LV$counts)
```
...
DF <- cbind(rownames(dge.BC1_PI.LV$samples), dge.BC1_PI.LV$samples)
colnames(DF) <- c("Sample", colnames(DF[2:ncol(DF)]))

# Saving the object 'DF' to be used for later creation of lib sizes plots for 
# the thesis
saveRDS(DF, file = "/home/fllobet/Documents/RNA-Seq_TFM/Extra_figures/eR_LIB/BC1_PI.LV.rds")

ggplot(DF[order(Animals.BC1_PI.LV$Group),], aes(x= `rownames(dge.BC1_PI.LV$samples)`, y = lib.size*1e-6, fill = group)) + geom_bar(stat = "identity", col = "#4d5d53") + labs(title = "Libray size accross samples", subtitle = st, x = "Sample", y = "Library size (millions)") + scale_x_discrete(breaks = rownames(dge.BC1_PI.LV$samples[order(Animals.BC1_PI.LV$Group),]), labels = str_replace(str_replace(rownames(dge.BC1_PI.LV$samples[order(Animals.BC1_PI.LV$Group),]), "X", ""),"_LV", "")) + 
...
dge.BC1_PI.LV$samples <- calcNormFactors(dge.BC1_PI.LV)
dge.BC1_PI.LV$samples

# Readdition of the group data because it gets removed in the previous step
dge.BC1_PI.LV$samples$group <- compare 

################## CHECKING COLS DGE object == ROWS metadata ###################
# The while loop serves to see the OK prompt when the orders has been modified. 
while (TRUE) {
  if (levels(as.factor(rownames(Animals.BC1_PI.LV) == 
                       colnames(dge.BC1_PI.LV))) == TRUE) {
    # If all the rows == all the columns (only TRUE): 
    print("The columns of the counts matrix are the same as 
          the rows of the metadata")
    break # Exit infinite loop. 
...
```{r common_dispersion}
dge.BC1_PI.LV <- estimateCommonDisp(dge.BC1_PI.LV)

# Value of the common dispersion:
dge.BC1_PI.LV$common.dispersion
sqrt(dge.BC1_PI.LV$common.dispersion)
```
...
df_all_pseudoCnt <- cbind(Animals.BC1_PI.LV, t(pseudoCnt.BC1_PI.LV))
...
```{r eda_05_MAplot}
# Manual MA-Plot
df_ma <- data.frame("A" = (pseudoCnt.BC1_PI.LV[,1] + pseudoCnt.BC1_PI.LV[, 2])/2,
                    "M" = pseudoCnt.BC1_PI.LV[,1] - pseudoCnt.BC1_PI.LV[,2])
...
df_ovd <- data.frame(mean = apply(cnt.BC1_PI.LV[, Animals.BC1_PI.LV$Group == "L"], 1, mean),
                     var = apply(cnt.BC1_PI.LV[, Animals.BC1_PI.LV$Group == "L"], 1, var))
...
df_hm <- as.matrix(dist(t(pseudoCnt.BC1_PI.LV)))
...
# SD for all the rows
rowSD <- apply(pseudoCnt.BC1_PI.LV, 1, sd)
# Ordering by SD
pseudoCnt.BC1_PI.LV.top500 <- pseudoCnt.BC1_PI.LV[order(rowSD,
                                                        decreasing = TRUE),]
# Selection of the top 500 more variable features
pseudoCnt.BC1_PI.LV.top500 <- pseudoCnt.BC1_PI.LV.top500[1:500,]
################################################################################
# Computation of the PCA
resPCA <- prcomp(pseudoCnt.BC1_PI.LV)
summary(resPCA)
resPCA$sdev
...
resPCA_pc12 <- cbind(resPCA_pc12, Animals.BC1_PI.LV)
...
PCA.plot <- ggplot(resPCA_pc12, aes(x = PC1, y = PC2, color = Group)) + 
  ...
  geom_text_repel(aes(label = str_replace(
    str_replace(rownames(Animals.BC1_PI.LV), "_LD", ""),"X", ""))) +
...
# OLD CODE - Wrong color assignation
# #plotMDS(pseudoCnt.BC1_PI.LV.top500, col = c(rgb(47/225,132/225,124/225),
...
#                                             rgb(189/225,183/225,107/225),
#                                             rgb(47/225,132/225,124/225),
#                                             rgb(189/225,183/225,107/225)), labels = str_replace(str_replace(colnames(pseudoCnt.BC1_PI.LV.top500), "X", ""),"_LV", ""))
...
mds <- plotMDS(pseudoCnt.BC1_PI.LV.top500, plot = FALSE)
...
mds.eigen <- mds.eigen[rownames(Animals.BC1_PI.LV),]

# Addition of group data
mds.eigen <- cbind(mds.eigen, Animals.BC1_PI.LV[order(rownames(mds.eigen)),7])
...
yuli.BC1_PI.LV <- read_ods("/home/fllobet/Documents/RNA-Seq_TFM/BC1_LD.LV/DEG_Yuliaxis_2012.ods")
yuli.BC1_PI.LV <- yuli.BC1_PI.LV[,c(1,3,4,5,6,7)]
colnames(yuli.BC1_PI.LV) <- c("ENSEMBL ID", "Gene name", colnames(yuli.BC1_PI.LV)[3:4])
# Exact test (core statistical test)
res_BC1_PI.LV <- exactTest(dge.BC1_PI.LV)
# Vislualitzation of the top 10 most significant features
topTags(res_BC1_PI.LV)

################################################################################
# FDR Correction: Benjamini-Honchberg.
res_BC1_PI.LV$table <- 
  cbind(res_BC1_PI.LV$table, 
        p.adjust(res_BC1_PI.LV$table$PValue, method = "BH"))

# Confirmation of the performed comparison
res_BC1_PI.LV$comparison

################################################################################
# Storing of the results into a CSV file
res_BC1_PI.LV$table <- cbind(rownames(res_BC1_PI.LV$table), res_BC1_PI.LV$table)
res_BC1_PI.LV$table <- cbind(rownames(res_BC1_PI.LV$table), res_BC1_PI.LV$table)
res_BC1_PI.LV$table[,1:2] <- str_split_fixed(rownames(res_BC1_PI.LV$table),"_", n = 2) 

colnames(res_BC1_PI.LV$table) <- c("ENSEMBL ID", "Gene name", colnames(res_BC1_PI.LV$table)[3:6])

# BUSCARV over the lists - ENSEMBL ID

IDs_in_yuli <- merge(res_BC1_PI.LV$table, yuli.BC1_PI.LV, by="ENSEMBL ID")$`ENSEMBL ID`

# Printing of the number of mathcing features
length(IDs_in_yuli)

# Addition of this info in the output CSV of the significant genes
L <- dim(res_BC1_PI.LV$table)[2]
for (i in 1:dim(res_BC1_PI.LV$table)[1]) {
  if (res_BC1_PI.LV$table[i,1] %in% IDs_in_yuli) {
    res_BC1_PI.LV$table[i,L+1] <- "in Ramayo-Caldas et al. 2012"
    }
}
################################################################################
...
GNs_in_yuli <- merge(res_BC1_PI.LV$table, yuli.BC1_PI.LV, by="Gene name")$`Gene name`
# Printing of the number of mathcing features
length(GNs_in_yuli)

L <- dim(res_BC1_PI.LV$table)[2]
for (i in 1:dim(res_BC1_PI.LV$table)[1]) {
  if (res_BC1_PI.LV$table[i, 2] %in% GNs_in_yuli) {
    res_BC1_PI.LV$table[i, L + 1] <- "in Ramayo-Caldas et al. 2012"
  }
}
...
colnames(res_BC1_PI.LV$table) <- c(colnames(res_BC1_PI.LV$table)[1:5], "PADJValue", "Paper ID", "Paper Gene")

# Merging the 2 columns of 'Paper ID' and 'Paper Name' to paint the plot
res_BC1_PI.LV$table  <- res_BC1_PI.LV$table %>% mutate(`Paper ID` = coalesce(`Paper ID`,`Paper Gene`)) %>%
         dplyr::select(colnames(res_BC1_PI.LV$table)[1:7])



################################################################################
# Storing the results into a CSV file 
colnames(res_BC1_PI.LV$table) <- c(colnames(res_BC1_PI.LV$table)[1:5], "PADJValue", "Paper")
write.table(res_BC1_PI.LV$table, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/edgeR/BC1_PI.LV_eResults.csv", sep = ";", row.names = FALSE)
```

```{r signficant_features}
# Return of significant features with a given alpha threshold:
signif_BC1_PI.LV <- decideTestsDGE(res_BC1_PI.LV, adjust.method = "BH", p.value = 0.05)

# Selection of the significant features
sel_signif_BC1_PI.LV <- which(signif_BC1_PI.LV[,1] != 0)

# Datafame of the Results + P.adjusted values of only the significant features
signif_BC1_PI.LV <- cbind(res_BC1_PI.LV$table[sel_signif_BC1_PI.LV,],
                          signif_BC1_PI.LV[sel_signif_BC1_PI.LV, 1])

################################################################################
# Printing of:
# 'n' of signficant features (rows)
dim(signif_BC1_PI.LV)[1]

# 'n' of logFC > 0 
dim(signif_BC1_PI.LV[which(signif_BC1_PI.LV$logFC > 0 & signif_BC1_PI.LV$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_PI.LV[which(signif_BC1_PI.LV$logFC < 0 & signif_BC1_PI.LV$PADJValue < 0.05),])[1]

# Storing the signficiant features into a CSV file
write.table(signif_BC1_PI.LV, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/edgeR/BC1_PI.LV_eResults_Signif.05.csv", sep = ";", row.names = FALSE)
```
...
# Filtering 
signif_BC1_PI.LV_FC_1.2 <- signif_BC1_PI.LV[which(signif_BC1_PI.LV$logFC > fc12 | signif_BC1_PI.LV$logFC < -fc12),]
dim(signif_BC1_PI.LV_FC_1.2)[1]

# 'n' of logFC > 0 
dim(signif_BC1_PI.LV_FC_1.2[which(signif_BC1_PI.LV_FC_1.2$logFC > 0 & signif_BC1_PI.LV_FC_1.2$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_PI.LV_FC_1.2[which(signif_BC1_PI.LV_FC_1.2$logFC < 0 & signif_BC1_PI.LV_FC_1.2$PADJValue < 0.05),])[1]
```
# ABS Fold-Change > 1.5
...
# Filtering 
signif_BC1_PI.LV_FC_1.5 <- signif_BC1_PI.LV[which(signif_BC1_PI.LV$logFC > fc15 | signif_BC1_PI.LV$logFC < -fc15),]
dim(signif_BC1_PI.LV_FC_1.5)[1]

# 'n' of logFC > 0 
dim(signif_BC1_PI.LV_FC_1.5[which(signif_BC1_PI.LV_FC_1.5$logFC > 0 & signif_BC1_PI.LV_FC_1.5$PADJValue < 0.05),])[1]
# 'n' of logFC < 0
dim(signif_BC1_PI.LV_FC_1.5[which(signif_BC1_PI.LV_FC_1.5$logFC < 0 & signif_BC1_PI.LV_FC_1.5$PADJValue < 0.05),])[1]
```
# PValues
...
```{r}
levels(as.factor(res_BC1_PI.LV$table$Paper))
ggplot(res_BC1_PI.LV$table[which(res_BC1_PI.LV$table$PADJValue < 1),], 
  ...
  scale_fill_manual(name = "", values = c("#ace1af"), na.value = "#bdb76b", 
                    labels = c("in Ramyo-Caldas et al. 2012", 
                               "NOT in Ramayo-Caldas et al. 2012")) + 
  labs(x = "Adjusted PValues distribution", y = "Frequency", title = "PValues distribution of BC1_PI ~ LIVER", subtitle = "Location of the signficiant features of Ramayo-Caldas et al. 2012 alognside the adjuste PValue distribution", caption = "The dashed lines repsent 0.05 and 0.1 respectively") + theme_bw() + 
...
CPM.BC1_PI.LV <- CPM.BC1_PI.LV[,rownames(Animals.BC1_PI.LV[order(Animals.BC1_PI.LV$Group, decreasing = TRUE),])]

# Division of the 'Features' column into 2 new columns
CPM.BC1_PI.LV <- cbind(rownames(CPM.BC1_PI.LV),
                       rownames(CPM.BC1_PI.LV),
                       CPM.BC1_PI.LV)

CPM.BC1_PI.LV[,1:2] <- str_split_fixed(rownames(CPM.BC1_PI.LV),"_", n = 2) 

colnames(CPM.BC1_PI.LV) <- c("ENSEMBL ID", "Gene name", str_replace(colnames(CPM.BC1_PI.LV)[3:dim(CPM.BC1_PI.LV)[2]],"X",""))

write.table(CPM.BC1_PI.LV, "/home/fllobet/Documents/RNA-Seq_TFM/BC1_PI.LV/edgeR/CPM.BC1_PI.LV.csv", sep = ";", row.names = FALSE) 
