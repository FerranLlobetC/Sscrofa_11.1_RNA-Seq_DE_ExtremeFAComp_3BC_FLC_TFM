...
echo "SAMPLE;Total;Unique;Unmapped;Duplicated;Insert_Size" > $PRJ.csv

# Iteration over all the '.log' files:
for LOG in *.log; do

    : " Total number of reads "
    F0=$(cat $LOG | grep "input reads " | cut -f 2)
    ...
    SAMPLE=$(echo $LOG | cut -f1 -d ".")    # delete longest match of pattern from the beginning
    ...
    echo "$SAMPLE;$F0;$F1;$F2;$F3;$IS" >> $PRJ.csv
done
