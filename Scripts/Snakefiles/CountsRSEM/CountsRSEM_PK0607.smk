...
rule all:
    input:
        expand("COUNTS/{file_name}.genes.results",  # The doc of RSEM told me what to expect as output <3 <3 <3.
            file_name = unique_to_rename(file_in=eq_csv, restrictions= ["USE-Standard"], col_out="SAMPLE_NAME"))
...
rule RSEM_counts:
    input:
        bm =  "ALIGNED_Reads/{file_name}.transcr.bam",  # Input BAM file.
