...
rule mapping:
    input:
        ...
        r1 = "PORK_02/{read_name}_1.fastq.gz",
        r2 = "PORK_02/{read_name}_2.fastq.gz",
    ...
    output:
        sm = "PORK_02/BAMs/{read_name}Aligned.out.bam"
...
rule BAM_merging:
    input:
        bm = expand("PORK_02/BAMs/{read_name}Aligned.out.bam",
              read_name=glob_wildcards("./PORK_02/{read_name}_1.fastq.gz").read_name)
    output:
        # (merged) BAMs
        bm1 = expand("PORK_02/BAMs/{file}.bam", file=unique_to_rename(file_in=eq_csv, col_out="SAMPLE_NAME"))
    log: bm_log = "PORK_02/BAMs/merged.bmlog"

    params:
        outdir = wkd + "PORK_02/BAMs/",  # The directory where the BAMs are (and will be).
        csv = subprocess.run(["./RNA-Seq_11.1_FLC/scripts/interchanger.py", eq_csv],  # Arguments for 'interchanger.py'.
...
rule summary:
    input: "PORK_02/BAMs/merged.bmlog"
    output: "PORK_02/summary.csv"
    params: outdir = wkd + "PORK_02/BAMs/"
