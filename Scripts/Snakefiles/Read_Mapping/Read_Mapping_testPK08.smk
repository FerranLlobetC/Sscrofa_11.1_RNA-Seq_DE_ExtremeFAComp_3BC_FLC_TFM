...
eq_csv = wkd + "RNA-Seq_11.1_FLC/08.csv"
workdir: wkd
...
rule all:
     input:
        "PORK_08/20190102/FASTQ/summary.csv"
...
rule genome_download:
    output:
        ref = "RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.Sscrofa11.1.dna.toplevel.fa.gz"
    run:
        # If the 'REF_Genome' directory does not exist:
        if not os.path.isdir("./RNA-Seq_11.1_FLC/REF_Genome"):
            shell("mkdir RNA-Seq_11.1_FLC/REF_Genome") # it is created.

        # Download of the reference genome.
        shell("cd ./RNA-Seq_11.1_FLC/REF_Genome")
        shell("wget http://ftp.ensembl.org/pub/release-105/fasta/sus_scrofa/dna_index/{output.ref}")

rule annot_download:
    output:
        annot = "RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.Sscrofa11.1.105.gtf.gz"
    run:
...
rule decompressions:
    input:
        ref = "RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.{genome}.dna.toplevel.fa.gz",  # Compressed genome.
        annot = "RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.{genome}.105.gtf.gz"  # Compressed annotations.
    output:
        s11 = "RNA-Seq_11.1_FLC/REF_Genome/{genome}.fa",  # De-compressed genome
        a11 = "RNA-Seq_11.1_FLC/REF_Genome/{genome}.gtf"  # De-compressed annotations.
    run:
        # Decompression
        shell("gzip -dk {input.ref}")    # Genome
        shell("gzip -dk {input.annot}")  # Gen annotations,
        # Name change
        shell("mv ./RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.{wildcards.genome}.dna.toplevel.fa {output.s11}")
        shell("mv ./RNA-Seq_11.1_FLC/REF_Genome/Sus_scrofa.{wildcards.genome}.105.gtf {output.a11}")

        # Removal of the compressed files
        shell("rm  ./RNA-Seq_11.1_FLC/REF_Genome/*.gz")
...
rule genome_index:
    input:
        s11 = "RNA-Seq_11.1_FLC/REF_Genome/Sscrofa11.1.fa",  # Genomic sequence.
        a11 = "RNA-Seq_11.1_FLC/REF_Genome/Sscrofa11.1.gtf"  # Annotations.
    ...
    output:
        ...
        chrNM= "RNA-Seq_11.1_FLC/INDEXED_Genome/chrName.txt",# One of the multiple files of the indexed genome.

    params:
        outdir = wkd + "RNA-Seq_11.1_FLC/INDEXED_Genome",  # The 'outdir' parameter is the directory where the indexed genome is stored.
    ...
    conda:
        "RNA-Seq_11.1_FLC/align.yaml"  # The 'align' conda environment will be used as it contains 'STAR' among others.
    shell:
...
rule mapping:
    input:
        ...
        r1 = "PORK_08/20190102/FASTQ/{read_name}_1.fastq.gz",
        r2 = "PORK_08/20190102/FASTQ/{read_name}_2.fastq.gz",
    ...
    output:
        sm = "PORK_08/20190102/FASTQ/BAMs/{read_name}Aligned.out.bam"

    params:
        genomedir = "RNA-Seq_11.1_FLC/INDEXED_Genome"
    threads: n_cores()
    ...
    shell:
        ...
        "./RNA-Seq_11.1_FLC/scripts/2_reads_mapB.sh {threads} {params.genomedir} {input.r1} {input.r2} {output.sm}"
...
rule BAM_merging:
    input:
        bm = expand("PORK_08/20190102/FASTQ/BAMs/{read_name}Aligned.out.bam", read_name=unique_to_rename(file_in=eq_csv,
            restrictions= ["USE-Test"],
            col_out="Filename"))
    output:
        # (merged) BAMs
        bm1 = expand("PORK_08/20190102/FASTQ/BAMs/{file}.bam", file=unique_to_rename(file_in=eq_csv,
            restrictions= ["USE-Test"],
            col_out="SAMPLE_NAME"))
    log: bm_log = "PORK_08/20190102/FASTQ/BAMs/merged.bmlog"

    params:
        outdir = wkd + "PORK_08/20190102/FASTQ/BAMs/",  # The directory where the BAMs are (and will be).
        csv = subprocess.run(["./RNA-Seq_11.1_FLC/scripts/interchanger.py", eq_csv, "USE-Test"],  # Arguments for 'interchanger.py'.
    ...
    shell:
         # Names replacing
         "./RNA-Seq_11.1_FLC/scripts/merge_rename_BAMs.py {params.outdir} {threads} {params.csv} {params.c_in} {params.c_out} {log}"
...
rule summary:
    input: "PORK_08/20190102/FASTQ/BAMs/merged.bmlog"
    output: "PORK_08/20190102/FASTQ/summary.csv"
    params: outdir = wkd + "PORK_08/20190102/FASTQ/BAMs/"
    shell: "./RNA-Seq_11.1_FLC/scripts/STAR_summary.sh {params.outdir}"
